 ...
 * Description: Enables the WordPress classic editor and the old-style Edit Post screen with
 * TinyMCE, Meta Boxes, etc. Supports the older plugins that extend this screen. Version:     1.5
 * Author:      WordPress Contributors Author URI:  https://github.com/WordPress/classic-editor/
 * License:     GPLv2 or later License URI: http://www.gnu.org/licenses/old-licenses/gpl-2.0.html
 * Text Domain: classic-editor Domain Path: /languages
 *
 * This program is free software; you can redistribute it and/or modify it under the terms of the
 * GNU General Public License version 2, as published by the Free Software Foundation. You may NOT
 * assume that you can use any other version of the GPL.
 *
...
if (!class_exists('Classic_Editor')) :
	class Classic_Editor{
		...
		public static function init_actions(){
			...
			if (!$settings['hide-settings-ui']){
				...
				add_filter('network_admin_plugin_action_links', [__CLASS__, 'add_settings_link'],
					10, 2);
			...
			if ($settings['allow-users']){
				...
				add_action('enqueue_block_editor_assets',
					[__CLASS__, 'enqueue_block_editor_scripts']);
			}else{
				if ($settings['editor'] === 'classic'){
					// Also used in Gutenberg.
			...
			if ($gutenberg){
				...
				remove_action('admin_print_scripts-edit.php',
					'gutenberg_replace_default_add_new_button');
				remove_filter('redirect_post_location',
					'gutenberg_redirect_to_classic_editor_when_saving_posts');
				remove_filter('display_post_states', 'gutenberg_add_gutenberg_post_state');
				remove_action('edit_form_top',
					'gutenberg_remember_classic_editor_when_saving_posts');
			}
		}
		...
		public static function remove_gutenberg_hooks($remove = 'all'){
			...
			remove_filter('wp_refresh_nonces',
			...
			remove_action('admin_enqueue_scripts',
				'gutenberg_check_if_classic_needs_warning_about_blocks');
		...
		private static function get_settings($refresh = 'no'){
			...
			if (is_multisite()){
				$defaults = [
					'editor'      => get_network_option(NULL,
						'classic-editor-replace') === 'block' ? 'block' : 'classic',
					'allow-users' => FALSE,
				];
		...
		public static function register_settings(){
			...
			add_option_whitelist([
				'writing' => ['classic-editor-replace', 'classic-editor-allow-users'],
			]);

			$heading_1 = __('Default editor for all users', 'classic-editor');
			$heading_2 = __('Allow users to switch editors', 'classic-editor');

			add_settings_field('classic-editor-1', $heading_1, [__CLASS__, 'settings_1'],
				'writing');
			add_settings_field('classic-editor-2', $heading_2, [__CLASS__, 'settings_2'],
				'writing');
		}
		...
		public static function settings_1(){
            ...
            <div class="classic-editor-options">
                <p>
                    ...
                    <label for="classic-editor-classic"><?php _ex('Classic Editor', 'Editor Name',
                ...
                <p>
                    ...
                    <label for="classic-editor-block"><?php _ex('Block Editor', 'Editor Name',
							'classic-editor'); ?></label>
                </p>
            </div>
            <script>
		...
		public static function settings_2(){
            ...
            <div class="classic-editor-options">
                ...
                <p>
                    ...
                    <label for="classic-editor-disallow"><?php _e('No',
							'classic-editor'); ?></label>
                </p>
            </div>
			<?php
		...
		public static function user_settings(){
            ...
            <table class="form-table">
                <tr class="classic-editor-user-options">
                    ...
                    <td>
						<?php wp_nonce_field('allow-user-settings',
							'classic-editor-user-settings'); ?><?php self::settings_1(); ?>
                    </td>
                </tr>
            </table>
		...
		public static function network_settings(){
            ...
            <table class="form-table">
				<?php wp_nonce_field('allow-site-admin-settings',
					'classic-editor-network-settings'); ?>
                <tr>
                    <th scope="row"><?php _e('Default editor for all sites',
							'classic-editor'); ?></th>
                    <td>
                        <p>
                            ...
                            <label for="classic-editor-classic"><?php _ex('Classic Editor',
									'Editor Name', 'classic-editor'); ?></label>
                        </p>
                        <p>
                            <input type="radio" name="classic-editor-replace" id="classic-editor-block" value="block"<?php if ($editor === 'block'){
								echo ' checked';
							} ?> />
                            <label for="classic-editor-block"><?php _ex('Block Editor',
									'Editor Name', 'classic-editor'); ?></label>
                        </p>
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        ...
                        <label for="classic-editor-allow-sites"><?php _e('Allow site admins to change settings',
								'classic-editor'); ?></label>
                        <p class="description"><?php _e('By default the Block Editor is replaced with the Classic Editor and users cannot switch editors.',
								'classic-editor'); ?></p>
                    </td>
                </tr>
            </table>
		...
		public static function save_network_settings(){
			if (
				...
				wp_verify_nonce($_POST['classic-editor-network-settings'],
					'allow-site-admin-settings')
			){
		...
		public static function redirect_location($location){
			if (
				...
				(isset($_POST['_wp_http_referer']) && strpos($_POST['_wp_http_referer'],
		...
		public static function do_meta_box($post){
            ...
            <p style="margin: 1em 0;">
                <a href="<?php echo esc_url($edit_url); ?>"><?php _e('Switch to Block Editor',
						'classic-editor'); ?></a>
            </p>
			<?php
		}
		...
		public static function add_edit_links($actions, $post){
			...
			$label      = sprintf(__('Edit &#8220;%s&#8221; in the Block Editor', 'classic-editor'),
				$title);
			$edit_block = sprintf('<a href="%s" aria-label="%s">%s</a>', esc_url($url),
				esc_attr($label), $text);
			...
			/* translators: %s: post title */
			$label        = sprintf(__('Edit &#8220;%s&#8221; in the Classic Editor',
				'classic-editor'), $title);
			$edit_classic = sprintf('<a href="%s" aria-label="%s">%s</a>', esc_url($url),
				esc_attr($label), $text);
		...
		public static function add_post_state($post_states, $post){
			...
			if (!$editors['classic_editor'] && !$editors['block_editor']){
				...
				$state = '<span class="classic-editor-forced-state">' . _x('Classic Editor',
						'Editor Name', 'classic-editor') . '</span>';
			}elseif (!$editors['classic_editor'] && $editors['block_editor']){
				// Forced to Block Editor.
				$state = '<span class="classic-editor-forced-state">' . _x('Block Editor',
						'Editor Name', 'classic-editor') . '</span>';
			}else{
				...
				$state = $is_classic ? _x('Classic Editor', 'Editor Name',
					'classic-editor') : _x('Block Editor', 'Editor Name', 'classic-editor');
			}
	...
	add_action('plugins_loaded', ['Classic_Editor', 'init_actions']);
