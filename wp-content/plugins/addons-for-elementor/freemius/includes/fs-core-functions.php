	...
	function fs_asset_url($asset_abs_path){
		...
		if (0 === strpos($asset_abs_path, $wp_content_dir)){
			...
			if (0 === strpos($asset_abs_path, $wp_plugins_dir)){
				...
				$asset_rel_path          = str_replace($wp_themes_dir, '',
					fs_normalize_path($asset_abs_path));
...
if (!function_exists('fs_enqueue_local_style')){
	function fs_enqueue_local_style($handle, $path, $deps = [], $ver = FALSE, $media = 'all'){
		wp_enqueue_style($handle, fs_asset_url(WP_FS__DIR_CSS . '/' . trim($path, '/')), $deps,
			$ver, $media);
	}
}

if (!function_exists('fs_enqueue_local_script')){
	function fs_enqueue_local_script(
		$handle,
		$path,
		$deps = [],
		$ver = FALSE,
		$in_footer = 'all'){
		wp_enqueue_script($handle, fs_asset_url(WP_FS__DIR_JS . '/' . trim($path, '/')), $deps,
			$ver, $in_footer);
	}
    }
...
if (!function_exists('fs_ui_get_action_button')){
	...
	function fs_ui_get_action_button(
		...
		if (is_string($confirmation)){
			...
			return sprintf('<a href="%s" class="%s">%s</a></form>',
				wp_nonce_url(freemius($module_id)->_get_admin_page_url($page,
					array_merge($params, ['fs_action' => $action])), $action),
				'button' . (!empty($button_class) ? ' ' . $button_class : '') . ($is_primary ? ' button-primary' : '') . ($is_small ? ' button-small' : ''),
				$title
			);
		}
	...
	function fs_ui_action_link($module_id, $page, $action, $title, $params = []){
		?><a class=""href="<?php echo wp_nonce_url(freemius($module_id)->_get_admin_page_url($page,
			array_merge($params, ['fs_action' => $action])),
			$action) ?>"><?php echo $title ?></a><?php
	}
}
