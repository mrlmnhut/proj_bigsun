...
------------------------------------------------------------------------------------------------------------------*/
	    ...
	    /**
	     * If `true` the opt-in should be shown as a modal dialog box on the themes.php page.
	     * WordPress.org themes guidelines prohibit from redirecting the user from the themes.php
	     * page after activating a theme.
	     *
	     * @return bool
	     * @since  2.4.5
	     *
        ...
        private function migrate_to_subscriptions_collection() {
            ...
            if ( isset( $this->_storage->subscription ) && is_object( $this->_storage->subscription ) ) {
	            $this->_storage->subscriptions = [fs_get_entity($this->_storage->subscription,
		            FS_Subscription::get_class_name())];
            }
        }
	    ...
	    private function register_constructor_hooks(){
		    ...
		    if ($this->_is_network_active && fs_is_network_admin()){
			    $this->add_ajax_action('network_activate',
				    [&$this, '_network_activate_ajax_action']);
		    }
	    ...
	    /**
	     * Makes Freemius-related updates unavailable on the "Add Plugins" admin page
	     * (/plugin-install.php) so that they won't interfere with the .org plugins'
	     * functionalities on that page (e.g. updating of a .org plugin).
	     *
            ...
            for ( $i = 1, $bt = debug_backtrace(), $len = count( $bt ); $i < $len; $i ++ ) {
                ...
                if ( ! isset( $caller_map[ $caller_file_hash ] ) ) {
                    foreach ( $all_plugins_paths as $plugin_path ) {
	                    ...
	                    if (FALSE !== strpos($caller_file_path,
			                    fs_normalize_path(dirname($plugin_path) . '/'))){
...
//				$parent_name = $this->get_option( $plugin_info['parent'], 'name', null );
            ...
            $plugin->update( array(
                ...
                'bundle_public_key'    => $this->get_option($plugin_info, 'bundle_public_key',
	                NULL),
            ));
...
//            }
        ...
        private static function get_account_option( $option_name, $module_type = null, $network_level_or_blog_id = null ) {
            ...
            return self::maybe_get_entities_account_option($option_name, [],
         ...
         */
	    private static function maybe_get_entities_account_option(
		    $option_name,
		    $default = NULL,
		    $network_level_or_blog_id = NULL){
		    $option = self::$_accounts->get_option($option_name, $default,
			    $network_level_or_blog_id);
        ...
        function _get_addon_info( $addon_id, $is_installed ) {
            ...
            if ( isset( $addon_info['license'] ) ) {
                if ( isset( $addon_storage->subscriptions ) &&
                    ...
                    $addon_subscriptions = fs_get_entities($addon_storage->subscriptions,
	                    FS_Subscription::get_class_name());
...
//            $this->_store_licenses()
		    ...
		    if (is_object($license)){
			    ...
			    if ($this->is_addon()){
				    ...
				    $this->get_parent_instance()
				         ->store_last_activated_license_data($license, $license_user);
			    }else{
	     ...
	     *
	     */
	    private function store_last_activated_license_data(
			    ...
			    if (!$this->has_addons()){
				    ...
				    if (!empty($addon_ids)){
					    ...
					    foreach ($addon_ids as $addon_id){
						    ...
						    $addon_storage = FS_Storage::instance(WP_FS__MODULE_TYPE_PLUGIN,
							    $addon->slug);
						    ...
						    if (is_object($fs_addon)){
							    ...
							    $was_addon_network_activated = $addon_storage->get('was_plugin_loaded',
								    FALSE, TRUE);
						    }
						    ...
						    if (
							    ...
							    foreach ($sites as $site){
								    ...
								    if ($addon_storage->get('is_whitelabeled', FALSE,
									    $site_info['blog_id'])){
									    $is_whitelabeled = TRUE;
									    break;
								    }
        ...
        function _get_subscription( $license_id ) {
            ...
            foreach ( fs_get_entities( $this->_storage->subscriptions,
	            FS_Subscription::get_class_name()) as $subscription){
        ...
        function store_subscription( FS_Subscription $subscription ) {
            ...
            $subscriptions = fs_get_entities( $this->_storage->subscriptions,
	            FS_Subscription::get_class_name());
	    ...
	    /**
	     * Displays the "Change User" dialog box when the user clicks on the "Change User" button
	     * on the "Account" page.
	     *
	    ...
	    /**
	     * Displays a subscription cancellation dialog box when the user clicks on the "Deactivate
	     * License" link on the "Account" page or deactivates a plugin and there's an active
	     * subscription that is either associated with a non-lifetime single-site license or
	     * non-lifetime multisite license that is only activated on a single production site.
	     *
	    ...
	    function _user_change_ajax_action(){
		    ...
		    if (empty($new_email_address) && !FS_User::is_valid_id($new_user_id)){
			    self::shoot_ajax_failure(fs_text_inline('Invalid new user ID or email address.',
				    'invalid-new-user-id-or-email', $this->get_slug()));
		    }
		    ...
		    $install = $this->get_api_site_scope()
		                    ->call($this->add_show_pending('/'), 'put', $params);
			    ...
			    if (is_object($install)){
				    switch ($install->error->code){
					    case 'user_exist':
						    $error = (
							    $this->get_text_x_inline('Oops', 'exclamation', 'oops') . '...' .
							    $this->get_text_inline('Sorry, we could not complete the email update. Another user with the same email is already registered.',
								    'user-exist-message') . ' ' .
							    sprintf($this->get_text_inline('If you would like to give up the ownership of the %s\'s account to %s click the Change Ownership button.',
								    'user-exist-message_ownership'), $this->_module_type,
								    '<b>' . $new_email_address . '</b>') .
							    sprintf(
								    '<a style="line-height: 40px;" href="%s"><button class="button button-primary">%s &nbsp;&#10140;</button></a>',
								    $this->get_account_url('change_owner', [
									    'state'           => 'init',
	    ...
	    /**
	     * A helper method to activate migrated licenses. If the product is network activated and
	     * integrated, the method will network activate the license.
	     *
	     * @param string $license_key
	     ...
	     * @author Vova Feldman (@svovaf)
	     * @since  2.2.4
	     * @since  2.0.0 When a super-admin that hasn't connected before is network activating a
	     *     license and excluding some of the sites for the license activation, go over the
	     *     unselected sites in the network and if a site is not connected, skipped, nor
	     *     delegated, if it's a freemium product then just skip the connection for the site, if
	     *     it's a premium only product, delegate the connection and license activation to the
	     *     site admin (Vova Feldman @svovaf).
	     * @var bool $success
	     * @var string $error
	     * @var string $next_page
	     * }
	    ...
	    private function activate_license(
		    ...
		    if (is_object($user)){
			    if (fs_is_network_admin() && !$has_valid_blog_id){
                    ...
                    if ( ! empty( $blog_2_install_map ) ) {
                        $result = $fs->activate_license_on_many_installs( $user, $license_key,
	                        $blog_2_install_map);
                    ...
                    if ( empty( $error ) && ! empty( $site_ids ) ) {
                        $result = $fs->activate_license_on_many_sites( $user, $license_key,
	                        $site_ids);
				    ...
				    if ($fs->is_registered()){
					    ...
					    if (FS_Api::is_api_error($install)){
						    ...
						    if (
							    ...
							    $fs->complete_ownership_change_by_license($install->user_id,
								    $install_ids);
						    }
					    }
	    ...
	    function _network_activate_ajax_action(){
		    ...
		    if (is_array($sites) && !empty($sites)){
			    ...
			    $error = $this->get_text_inline('Invalid site details collection.',
				    'invalid_site_details_collection');
		    }
	    ...
	    function switch_to_blog($blog_id, FS_Site $install = NULL){
		    ...
		    $this->_storage->set_network_active($this->_is_network_active,
			    $this->is_delegated_connection($blog_id));
	    ...
	    /**
	     * Check if currently on the product's main setting page or on any of the Freemius added
	     * pages (via tabs).
	     *
	     * @author Vova Feldman (@svovaf)
	     * @since  1.2.2.7
...
//				check_admin_referer( $this->_slug . '_activate_new' );
	    ...
	    private function install_many_with_new_user(
		    ...
		    $installs = [];
		    while ($left > 0){
			    $result = $this->get_api_user_scope()
			                   ->get("/plugins/{$this->_module_id}/installs.json?ids=" . implode(',',
					                   array_slice($install_ids, $offset, 25)));
...
//				check_admin_referer( 'activate_existing_' . $this->_plugin->public_key );
	     ...
	     * @param bool $redirect
	     *
	     * @return object|string If redirect is `false`, returns the next page the user should be
	     *     redirected to, or the API error object if failed to install.
	     */
...
//			}
        ...
        private function _store_site( $store = true, $network_level_or_blog_id = null, FS_Site $site = null ) {
            ...
            if (is_object($this->_user) && $this->_user->id != $site->user_id ) {
	            ...
	            $prev_stored_user_id = $this->_storage->get('prev_user_id', FALSE,
		            $network_level_or_blog_id);
	            ...
	            if (empty($prev_stored_user_id) &&
		             ...
		             * @author Leo Fajardo (@leorw)
		             */
		            $this->_storage->store('prev_user_id', $sites[$this->_slug]->user_id,
			            $network_level_or_blog_id);
	            }
            }
	     ...
	     * @return array
	     */
	    private function get_foreign_licenses_info(
		    $all_licenses,
		    $site_license_id = NULL,
		    $include_parent_licenses = FALSE){
		    $foreign_licenses = array(
                'ids'          => array(),
                'license_keys' => array()
            );
                ...
                if ( ! $is_valid ) {
                    ...
                    if ( ! $background || $this->is_paying() ) {
                        ...
                        if ( ! FS_Api::test() ) {
                            ...
                            $this->_admin_notices->add(
	                            $this->get_text_inline( 'It seems like one of the authentication parameters is wrong. Update your Public Key, Secret Key & User ID, and try again.',
		                            'wrong-authentication-param-message') . '<br> ' . $this->get_text_inline('Error received from the server:',
		                            'server-error-message') . var_export($result->error, TRUE),
	                            $this->get_text_x_inline('Oops', 'exclamation', 'oops') . '...',
	                            'error'
                            );
                        }
                    ...
                    if ( ! is_object( $this->_license ) ) {
	                    ...
	                    if ($this->_license->is_expired()){
		                    if (!$this->has_features_enabled_license()){
			                    ...
			                    $last_time_expired_license_notice_was_shown = $this->_storage->get('expired_license_notice_shown',
				                    0);
            ...
            if ( $this->apply_filters( 'has_paid_plan_account', $this->has_paid_plan() ) ) {
                switch ( $plan_change ) {
	                ...
	                case 'trial_started':
		                $this->_admin_notices->add_sticky(
			                sprintf(
				                $this->get_text_inline('Your trial has been successfully started.',
					                'trial-started-message'),
				                '<i>' . $this->get_plugin_name() . '</i>'
			                ) . $this->get_complete_upgrade_instructions($this->get_trial_plan()->title),
			                'trial_started',
			                $this->get_text_x_inline('Yee-haw',
				                'interjection expressing joy or exuberance', 'yee-haw') . '!'
		                );
            ...
            if ( $show_notice ) {
                $this->_admin_notices->add(
                    sprintf( $this->is_only_premium() ?
                        $this->get_text_inline( 'Your %s license was successfully deactivated.',
	                        'license-deactivation-message_premium-only') :
	                    $this->get_text_inline('Your license was successfully deactivated, you are back to the %s plan.',
		                    'license-deactivation-message'),
	                    $this->get_plan_title()
                    ),
