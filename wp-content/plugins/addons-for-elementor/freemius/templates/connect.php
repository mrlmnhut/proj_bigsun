...
<div id="fs_theme_connect_wrapper">
	...
	<div id="fs_connect"
		...
		/**
		 * When activating a license key the information of the admin is not collected, we gather
		 * the user info from the license.
		 *
		 * @since 2.3.2
		 * @author Vova Feldman
		 */
		...
		$permissions['site'] = [
			...
			'desc'       => $fs->get_text_inline('Site URL, WP version, PHP info',
				'permissions-site_desc'),
			'priority'   => 10,
		];
		...
		$permissions['notices'] = [
			...
			'desc'       => $fs->get_text_inline('Updates, announcements, marketing, no spam',
				'permissions-newsletter_desc'),
			'priority'   => 13,
		];

		$permissions['events'] = [
			'icon-class' => 'dashicons dashicons-admin-' . ($fs->is_plugin() ? 'plugins' : 'appearance'),
			'label'      => sprintf($fs->get_text_inline('Current %s Events', 'permissions-events'),
				ucfirst($fs->get_module_type())),
			'desc'       => $fs->get_text_inline('Activation, deactivation and uninstall',
				'permissions-events_desc'),
			'priority'   => 20,
		];
		...
		$permissions['extensions'] = [
			...
			'desc'       => $fs->get_text_inline('Title, slug, version, and is active',
				'permissions-extensions_desc'),
			'priority'   => 25,
			'optional'   => TRUE,
		];
