...
<tr<?php if ( $odd ) {
    ...
    <?php if ( $is_addon_connected ) : ?>
        ...
        <!-- Expiration -->
        <td>
	        <?php if ($site->is_trial() || is_object($license)) : ?><?php
		        $tags = [];

		        if ($site->is_trial()){
			        $tags[] = ['label' => $trial_text, 'type' => 'success'];
	    ...
	    if ($is_addon_activated){
		    if (!$is_whitelabeled){
			    if ($is_paying){
				    ...
				    $human_readable_license_expiration = human_time_diff(time(),
				    ...
				    $after_downgrade_message = !$license->is_block_features ?
					    sprintf($after_downgrade_non_blocking_text, $plan->title,
						    $fs_addon->get_module_label(TRUE)) :
				    ...
				    if (!$license->is_lifetime() && $is_active_subscription){
					    $buttons[] = fs_ui_get_action_button(
						    ...
						    'account',
						    'downgrade_account',
						    esc_html($fs_addon->is_only_premium() ? fs_text_inline('Cancel Subscription',
							    'cancel-subscription', $slug) : $downgrade_text),
						    '',
						    ['plugin_id' => $addon_id],
						    FALSE,
						    FALSE,
				    ...
				    if (!empty($premium_licenses)){
					    ...
					    if (!$has_multiple_premium_licenses){
						    ...
						    $buttons[] = fs_ui_get_action_button(
							    ...
							    'account',
							    'activate_license',
							    esc_html(sprintf($activate_plan_text, $premium_plan->title,
								    ($site->is_localhost() && $premium_license->is_free_localhost) ? '[localhost]' : (1 < $premium_license->left() ? $premium_license->left() . ' left' : ''))),
    ...
    <?php else : ?>
        ...
        <td colspan="<?php echo ($is_addon_installed_by_filesystem || $show_delete_install_button) ? '5' : '4' ?>">
		    <?php if ($is_addon_installed_by_filesystem) : ?><?php $addon_file = $fs->get_addon_basename($addon_id) ?><?php if (!isset($active_plugins_directories_map[ dirname( $addon_file ) ] ) ) : ?>
