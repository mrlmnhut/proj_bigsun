...
<div>
    ...
    <span class="fs-switch-label"><?php fs_esc_html_echo_x_inline('Debugging',
			'as code debugging') ?></span>
...
<?php foreach ($module_types as $module_type) : ?><?php $modules = fs_get_entities($fs_options->get_option($module_type . 's'),
                        ...
                        <td><?php
                                ...
                                if ( FS_Plugin_Plan::is_valid_id( $site->plan_id ) ) {
                                    if ( false === $all_plans ) {
	                                    ...
	                                    $all_plans = fs_get_entities($fs_options->get_option($option_name,
...
<?php if ( is_array( $users ) && 0 < count( $users ) ) : ?>
    ...
    <table id="fs_users" class="widefat">
        ...
        <tbody>
        <?php foreach ($users as $user_id => $user) : ?><?php $has_developer_license = isset($users_with_developer_license_by_id[$user_id]) ?>
            <tr>
                ...
                <td>
	                <?php if (!$has_developer_license) : ?>
                        <form action="" method="POST">
                            ...
                            <button type="submit" class="button"><?php fs_esc_html_echo_x_inline('Delete',
					                'verb', 'delete') ?></button>
                        </form>
	                <?php endif ?>
                </td>
...
<?php endif ?>
<?php foreach ( $module_types as $module_type ) : ?><?php
	/**
	 * @var FS_Plugin_License[] $licenses
	 */
	$licenses = $VARS[$module_type . '_licenses'] ?><?php if (is_array($licenses) && count($licenses) > 0) : ?>
