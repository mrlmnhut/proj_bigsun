...
if (!class_exists('Brainstorm_Update_UAEL')) :
	...
	class Brainstorm_Update_UAEL{
		...
		public function __construct(){
			...
			add_filter('bsf_get_license_message_uael', [$this, 'license_message_uael'], 10, 2);
			add_filter('bsf_skip_braisntorm_menu', [$this, 'skip_menu']);
			add_filter('bsf_skip_author_registration', [$this, 'skip_menu']);
			add_filter('bsf_allow_beta_updates_uael', [$this, 'beta_updates_check']);
			// Register Licence Link.
			add_filter('bsf_registration_page_url_uael', [$this, 'get_registration_page_url']);
			add_filter('agency_updater_productname_uael', [$this, 'product_name']);

			// Add popup license form on plugin list page.
			add_filter('plugin_action_links_' . UAEL_BASE,
				[$this, 'plugin_slug_license_form_and_links']);
			add_filter('network_admin_plugin_action_links_' . UAEL_BASE,
				[$this, 'plugin_slug_license_form_and_links']);
		}
		 ...
		 */
		function plugin_slug_license_form_and_links($links = []){
		...
		function license_message_uael($content, $purchase_url){
			...
			$message = "<p><a target='_blank' href='" . esc_url($purchase_url) . "'>" . esc_html__('Get the license >>',
					'uael') . '</a></p>';
