...
if (!class_exists('UAEL_Loader')){
	...
	final class UAEL_Loader{
		...
		public function __construct(){
			...
			// Activation hook.
			register_activation_hook(UAEL_FILE, [$this, 'activation_reset']);

			// deActivation hook.
			register_deactivation_hook(UAEL_FILE, [$this, 'deactivation_reset']);
		...
		function load_plugin(){
			...
			if (!did_action('elementor/loaded')){
				/* TO DO */
				add_action('admin_notices', [$this, 'uael_fails_to_load']);
		...
		public function uael_fails_to_load(){
			...
			$message = sprintf(__('The %1$sUltimate Addon for Elementor%2$s plugin requires %1$sElementor%2$s plugin installed & activated.',
				'uael'), '<strong>', '</strong>');
			...
			if (_is_elementor_installed()){
				...
				$action_url   = wp_nonce_url('plugins.php?action=activate&amp;plugin=' . $plugin . '&amp;plugin_status=all&amp;paged=1&amp;s',
				...
				$action_url   = wp_nonce_url(self_admin_url('update.php?action=install-plugin&plugin=elementor'),
