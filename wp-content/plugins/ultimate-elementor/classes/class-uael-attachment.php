...
if (!class_exists('UAEL_Attachment')){
	...
	class UAEL_Attachment{
		...
		function __construct(){
			...
			add_filter('attachment_fields_to_edit', [$this, 'custom_attachment_field_link'], 10, 2);
			add_filter('attachment_fields_to_save', [$this, 'custom_attachment_field_link_save'],
				10, 2);
		}
		...
		function custom_attachment_field_link($form_fields, $post){
			...
			$form_fields['uael-categories'] = [
				/* translators: %1$s: uael name. */
				'label' => sprintf(__('%1s - Categories (Ex: Cat1, Cat2)', 'uael'),
