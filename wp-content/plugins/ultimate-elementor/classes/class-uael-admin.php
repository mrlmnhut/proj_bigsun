	...
	final class UAEL_Admin{
		...
		public static function uael_whitelabel_description(){
			...
			if (empty($branding_desc)){
				$branding_desc = __('Ultimate Addons is a premium extension for Elementor that adds 30+ widgets and works on top of any Elementor Package (Free, Pro). You can use it with any WordPress theme.',
					'uael');
			}
		...
		public static function load_admin(){
			add_action('elementor/editor/after_enqueue_styles',
				__CLASS__ . '::uael_admin_enqueue_scripts');
		}
		...
		static public function render(){
			...
			$uael_header_wrapper_class = apply_filters('uael_header_wrapper_class', [$action]);
		...
		static public function save_integration_option(){
			...
			if (isset($_POST['uael-integration-nonce']) && wp_verify_nonce($_POST['uael-integration-nonce'],
					'uael-integration')){
		...
		static public function save_branding_option(){
			...
			if (isset($_POST['uael-white-label-nonce']) && wp_verify_nonce($_POST['uael-white-label-nonce'],
					'white-label')){
		...
		static public function styles_scripts(){
			...
			// Styles.
			wp_enqueue_style('uael-admin-settings',
				UAEL_URL . 'admin/assets/admin-menu-settings.css', [], UAEL_VER);
			// Script.
			wp_enqueue_script('uael-admin-settings',
				UAEL_URL . 'admin/assets/admin-menu-settings.js', ['jquery', 'wp-util', 'updates'],
				UAEL_VER);
			...
			wp_localize_script('uael-admin-settings', 'uael',
				apply_filters('uael_js_localize', $localize));
		}
		...
		static public function initialize_ajax(){
			...
			add_action('wp_ajax_uael_bulk_deactivate_widgets',
				__CLASS__ . '::bulk_deactivate_widgets');
