	...
	class BSF_Update_Manager{
		...
		public function __construct(){
			...
			add_filter('pre_set_site_transient_update_themes',
				[$this, 'brainstorm_update_themes_transient']);

			// display changelog in update details
			add_filter('plugins_api', [$this, 'bsf_get_plugin_information'], 10, 3);

			// display correct error messages
			add_action('load-plugins.php', [$this, 'bsf_update_display_license_link']);
			add_filter('upgrader_pre_download', [$this, 'bsf_change_no_package_message'], 20, 3);
		}
		...
		public function brainstorm_update_plugins_transient($_transient_data){
			...
			foreach ($update_data as $key => $product){
				...
				$plugin->icons = apply_filters("bsf_product_icons_{$product['id']}", [
		...
		public function bsf_get_plugin_information($_data, $_action = '', $_args = NULL){
			...
			$brainstrom_products = apply_filters('bsf_get_plugin_information',
				get_option('brainstrom_products', []));
			...
			foreach ($all_products as $key => $product){
				...
				if ($product_slug == $_args->slug){
					...
					$info->url      = isset($product['changelog_url']) ? apply_filters("bsf_product_url_{$id}",
						$product['changelog_url']) : apply_filters("bsf_product_url_{$id}", '');
					$info->homepage = isset($product['purchase_url']) ? apply_filters("bsf_product_homepage_{$id}",
						$product['purchase_url']) : apply_filters("bsf_product_homepage_{$id}", '');
					...
					$info->sections['description'] = apply_filters("bsf_product_description_{$id}",
						$product_decription);
					...
					$info->sections['changelog']   = apply_filters("bsf_product_changelog_{$id}",
						$product_changelog);
		...
		public static function bsf_is_product_bundled($bsf_product, $search_by = 'id'){
			...
			$product_parent = apply_filters('bsf_is_product_bundled', array_unique($product_parent),
				$bsf_product, $search_by);
		...
		public function bsf_update_transient_data($product_type){
			...
			if ($product_type == 'plugins'){
				$all_products = $this->prepare_plugins_for_update(brainstorm_get_all_products(FALSE,
					TRUE, FALSE));
			}
		...
		public function brainstorm_all_products(){
			...
			$brainstrom_products_plugins = isset($brainstrom_products['plugins']) ? $brainstrom_products['plugins'] : [];
			$brainstrom_products_themes  = isset($brainstrom_products['themes']) ? $brainstrom_products['themes'] : [];
			$brainstrom_bundled_products = get_option('brainstrom_bundled_products', []);
		...
		public function bsf_add_registration_message($plugin_data, $response){
			...
			if (!empty($bundled)){
				...
				$parent_name       = apply_filters("bsf_product_name_{$parent_id}",
					brainstrom_product_name($parent_id));
				$message           = sprintf(__(' <br>This plugin is came bundled with the <i>%1$s</i>. For receiving updates, you need to register license of <i>%2$s</i> <a href="%3$s">here</a>.'),
					$parent_name, $parent_name, $registration_page);
			}else{
				$message = sprintf(__(' <i>Please <a href="%1$s">activate your license</a> to update the plugin.</i>'),
					$registration_page);
			}
		...
		public function bsf_change_no_package_message($reply, $package, $current){
			...
			if (isset($current->skin->plugin_info)){
				...
				$product_id            = brainstrom_product_id_by_name($plugin_name);
				$plugin_name           = apply_filters("bsf_product_name_{$product_id}",
					$plugin_name);
				...
				if (empty($is_bundled)){
					...
					$plugin_name           = apply_filters("bsf_product_name_{$is_bundled}",
						brainstrom_product_name($is_bundled));
				...
				if ($plugin_info['Author'] == 'Brainstorm Force'){
					...
					$plugin_init = BSF_License_Manager::instance()
					                                  ->bsf_get_product_info($product_id,
						                                  'template');
