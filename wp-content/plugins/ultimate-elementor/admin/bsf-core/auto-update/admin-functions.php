	...
	function bsf_deregister_product_callback(){
		...
		if (trim($purchase_key) == ''){
			if (class_exists('BSF_License_Manager')){
				...
				$purchase_key        = $bsf_license_manager->bsf_get_product_info($id,
					'purchase_key');
			}
		}
...
/**
 * Update version numbers of all the brainstorm products in options `brainstorm_products` and
	...
	function bsf_notices(){
		...
		if ($pagenow === 'plugins.php' || $pagenow === 'post-new.php' || $pagenow === 'edit.php' || $pagenow === 'post.php'){
			...
			foreach ($mix as $product) :
				...
				if (FALSE == apply_filters("bsf_display_product_activation_notice_{$product['id']}",
						TRUE)){
					continue;
				}
				...
				if ($status !== 'registered') :
					...
					$message = __('Please',
							'bsf') . ' <a href="' . $url . '" class="bsf-core-license-form-btn" plugin-slug="' . $product['id'] . '">' . __('activate',
							'bsf') . '</a> ' . __('your copy of the',
							'bsf') . ' <i>' . $product['product_name'] . '</i> ' . __('to get update notifications, access to support features & other resources!',
							'bsf');
					$message = apply_filters("bsf_product_activation_notice_{$product['id']}",
						$message, $url, $product['product_name']);
		...
		if ($response){
			if ($action === 'grant'){
				...
				if (update_option('developer_access', FALSE)){
					...
					?>
                    <div class="error"><p><?php echo __('Something went wrong. Please try again!',
								'bsf'); ?></p></div>
					<?php
				}
			}
...
if (!function_exists('brainstorm_switch')){
	...
	function brainstorm_switch($name = '', $default = FALSE){
		...
		$switch .= "\t\t" . '<div class="switch-wrapper">
				...
				FALSE) . ' id="brainstorm_core_switch_' . $uid . '" name="' . $name . '" class="ios-toggle smile-input bsf-switch-input switch-checkbox smile-switch " value="0" >
