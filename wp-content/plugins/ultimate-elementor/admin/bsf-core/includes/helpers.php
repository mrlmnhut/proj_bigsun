...
function bsf_get_api_site($prefer_unsecure = FALSE){
	...
	if (defined('BSF_API_URL')){
		...
		if (FALSE == $prefer_unsecure && wp_http_supports(['ssl'])){
...
if (!function_exists('brainstorm_get_all_products')){
	...
	function brainstorm_get_all_products(
		$skip_plugins = FALSE,
		$skip_themes = FALSE,
		$skip_bundled = FALSE){
			...
			foreach ($brainstrom_bundled_products as $parent_id => $parent){
				...
				foreach ($parent as $key => $product){
					...
					if (isset($all_products[$product->id])){
						$all_products[$product->id] = array_merge($all_products[$product->id],
							(array) $product);
					}else{
	...
	function bsf_extension_nag($product_id = '', $mu_updater = FALSE){
		...
		if ($bsf_not_activated_plugins !== '' || $bsf_not_installed_plugins !== ''){
			...
			if ($bsf_not_installed_plugins !== ''){
				echo '<p>';
				echo $bsf_product_name . __(' requires following plugins to be installed and activated : ',
						'bsf');
				echo '<strong><em>';
			...
			if ($bsf_activate == TRUE){
				$installer .= '<a href="' . get_admin_url() . 'plugins.php?plugin_status=inactive">' . __('Begin activating plugins',
						'bsf') . '</a> | ';
			}

			if ($bsf_install == TRUE){
				$installer .= '<a href="' . bsf_exension_installer_url($product_id) . '">' . __('Begin installing plugins',
						'bsf') . '</a> | ';
			}

			$installer .= '<a href="' . esc_url(add_query_arg('bsf-dismiss-notice',
					$product_id)) . '">' . __('Dismiss This Notice', 'bsf') . '</a>';
...
if (!function_exists('bsf_nag_brainstorm_updater_multisite')){
	...
	function bsf_nag_brainstorm_updater_multisite(){
		...
		printf(
			__('Looks like you are on a WordPress Multisite, you will need to install and network activate %1$s Brainstorm Updater for Multisite %2$s plugin. Download it from %3$s here %4$s',
				'bsf'),
			'<strong><em>',
			'</strong></em>',
