...
class BSF_Envato_Activate{
	...
	public function __construct(){
		...
		add_filter('update_footer', [$this, 'alternate_method_link'], 20);
		add_action('bsf_inlne_license_envato_after_form', [$this, 'inline_alternate_method_link'],
			20, 2);
	}
	...
	public function envato_register($args){
		...
		if ($is_active){
			...
			$envato_active_oauth_title    = apply_filters("envato_active_oauth_title_{$product_id}",
				'Updates & Support Registration - <span class="active">Active!</span>');
			$envato_active_oauth_subtitle = '<span class="active">' . sprintf(
					'Your license is active.',
					$product_name
				) . '</span>';

			$envato_active_oauth_subtitle = apply_filters("envato_active_oauth_subtitle_{$product_id}",
			...
			$envato_not_active_oauth_title    = apply_filters("envato_not_active_oauth_title_{$product_id}",
				__('Updates & Support Registration - <span class="not-active">Not Active!</span>',
					'bsf'));
			$envato_not_active_oauth_subtitle = apply_filters("envato_not_active_oauth_subtitle_{$product_id}",
				__('Click on the button below to activate your license and subscribe to our newsletter.',
					'bsf'));
			...
			$html .= '<input type="button" class="button ' . $submit_button_class . '" name="bsf_activate_license" value="' . esc_attr__($button_text_activate,
					'bsf') . '"/>';
	...
	protected function get_redirect_url($product_id = ''){
		...
		$current_url = esc_url(remove_query_arg(['license_action', 'token', 'product_id', 'purchase_key', 'success', 'status', 'message'],
			$current_url));
	...
	public function inline_alternate_method_link($html, $bsf_product_id){
		$privacy_policy_link   = $this->license_manager->bsf_get_product_info($bsf_product_id,
			'privacy_policy');
		$terms_conditions_link = $this->license_manager->bsf_get_product_info($bsf_product_id,
			'terms_conditions');
