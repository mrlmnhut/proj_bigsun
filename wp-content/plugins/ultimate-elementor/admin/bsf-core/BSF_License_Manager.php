...
if (!class_exists('BSF_License_Manager')){
	...
	class BSF_License_Manager{
		...
		private static $inline_form_products = [];
		...
		public function __construct(){
			...
			add_action('admin_init', [$this, 'bsf_activate_license']);
			add_action('admin_init', [$this, 'bsf_deactivate_license']);
			add_action('bsf_product_update_registered',
				[$this, 'refresh_products_on_license_activae']);
			add_action('admin_footer', [$this, 'render_popup_form_markup']);
		...
		public function bsf_deactivate_license(){
			...
			if (!is_wp_error($response) || wp_remote_retrieve_response_code($response) === 200){
				...
				if (isset($result['success']) && $result['success'] == TRUE){
					...
					do_action('bsf_deactivate_license_' . $product_id . '_after_success', $result,
						$response, $_POST);
		...
		public function bsf_activate_license(){
			...
			if (!is_wp_error($response) || wp_remote_retrieve_response_code($response) === 200){
				...
				if (isset($result['success']) && $result['success'] == TRUE){
					...
					do_action('bsf_activate_license_' . $product_id . '_after_success', $result,
						$response, $_POST);
				}else{
					$_POST['bsf_license_activation']['success'] = $result['success'];
					$_POST['bsf_license_activation']['message'] = $result['message'];
		...
		public static function bsf_is_active_license($product_id){
			...
			$brainstrom_products = get_option('brainstrom_products', []);
			$brainstorm_plugins  = isset($brainstrom_products['plugins']) ? $brainstrom_products['plugins'] : [];
			$brainstorm_themes   = isset($brainstrom_products['themes']) ? $brainstrom_products['themes'] : [];
			...
			if (isset($all_products[$product_id])){
				...
				if (isset($all_products[$product_id]['status']) && $all_products[$product_id]['status'] == 'registered'){
					...
					if (FALSE === self::instance()
					                  ->get_remote_license_status($all_products[$product_id]['purchase_key'],
						                  $product_id)){
						return FALSE;
					}
		...
		public static function is_product_free($product_id){
			...
			$is_free         = $license_manager->bsf_get_product_info($product_id,
				'is_product_free');
		...
		public function license_activation_form($args){
			...
			$product_name = apply_filters("agency_updater_productname_{$product_id}",
				$this->bsf_get_product_info($product_id, 'name'));
			if (empty($product_name)){
				$product_name = apply_filters("agency_updater_productname_{$product_id}",
					$this->bsf_get_product_info($product_id, 'product_name'));
			}
			...
			if ($is_active == FALSE){
				...
				$not_activate         = '';
				$html                 .= apply_filters("bsf_license_not_activate_message_{$product_id}",
					$not_activate, $license_status_class, $license_not_activate_message);

				if ($bsf_license_allow_email == TRUE){
					$popup_license_subtitle = apply_filters("bsf_license_key_form_inactive_subtitle_{$product_id}",
						__('<p>Click on the button below to activate your license and subscribe to our newsletter.</p>',
							'bsf'));
				}else{
					$popup_license_subtitle = apply_filters("bsf_license_key_form_inactive_subtitle_{$product_id}",
						__('<p>Enter your purchase key and activate automatic updates.</p>',
							'bsf'));
				}
			}else{
			...
			if ($is_active == TRUE){
				...
				$licnse_active_message = apply_filters('bsf_license_active_message',
				...
				$html .= '<input type="submit" class="button ' . $submit_button_class . '" name="bsf_deactivate_license" value="' . esc_attr__($button_text_deactivate,
						'bsf') . '"/>';
			}else{
				...
				$html .= '<input type="submit" class="button ' . $submit_button_class . '" name="bsf_activate_license" value="' . esc_attr__($button_text_activate,
						'bsf') . '"/>';
				...
				$html .= apply_filters("bsf_get_license_message_{$product_id}",
					$get_license_message, $purchase_url);
				$html .= '</div>';
			}
		...
		function load_scripts($hook = ''){
			...
			if ('plugins.php' === $hook){
				wp_register_script('bsf-core-jquery-history',
					bsf_core_url('/assets/js/jquery-history.js'), ['jquery'], BSF_UPDATER_VERSION,
					TRUE);
				wp_enqueue_style('bsf-core-license-form',
					bsf_core_url('/assets/css/license-form-popup.css'), [], BSF_UPDATER_VERSION,
					'all');
				wp_enqueue_script('bsf-core-license-form',
					bsf_core_url('/assets/js/license-form-popup.js'),
					['jquery', 'bsf-core-jquery-history'], BSF_UPDATER_VERSION, TRUE);
			}
		...
		public function get_bsf_inline_license_form($links, $args, $license_from_type){
			...
			if (is_multisite() && !is_network_admin() && FALSE == apply_filters("bsf_core_popup_license_form_per_network_site_{$product_id}",
					FALSE)){
				return $links;
			}
			...
			$form_args = [
				...
				'license_form_title'               => '',
				'license_deactivate_status'        => esc_html__('Your license is not active!',
					'bsf-core'),
				'license_activate_status'          => esc_html__('Your license is activated!',
					'bsf-core'),
                ...
                <div plugin-slug="<?php echo esc_attr($product_id); ?>" class="bsf-core-license-form" style="display: none;">
                    ...
                    <div class="bsf-core-license-form-inner">
                        <button type="button" class="bsf-core-license-form-close-btn">
                            <span class="screen-reader-text"><?php esc_html_e('Close',
									'bsf-core'); ?></span>
						...
						do_action("bsf_inlne_license_form_footer_{$product[ 'license_from_type' ]}",
							$product_id);
...
function get_bsf_inline_license_form($links, $bsf_product_id, $license_from_type){
	...
	return $license_manager->get_bsf_inline_license_form($links, $bsf_product_id,
		$license_from_type);
}
