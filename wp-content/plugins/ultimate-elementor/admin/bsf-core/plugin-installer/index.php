...
<div class="wrap">
	...
	?>
	<?php if (isset($installed['status']) && $installed['status'] === TRUE) : ?><?php $current_name = strtolower(bsf_get_current_name($installed['init'],
		$installed['type'])); ?><?php $current_name = preg_replace('![^a-z0-9]+!i', '-',
		$current_name); ?>
    <a href="<?php echo (is_multisite()) ? network_admin_url('plugins.php#' . $current_name) : admin_url('plugins.php#' . $current_name); ?>"><?php echo __('Manage plugin here',
		'bsf'); ?></a>
	<?php endif; ?>
</div>        <?php
...
$extensions_installer_heading = apply_filters("bsf_extinstaller_heading_{$product_id}",
	'iMedica Extensions');

$extensions_installer_subheading = apply_filters("bsf_extinstaller_subheading_{$product_id}",
	'iMedica is already very flexible & feature rich theme. It further aims to be all-in-one solution for your WordPress needs. Install any necessary extensions you like from below and take it on the steroids.');
...
<div class="wrap about-wrap bsf-sp-screen bend <?php echo 'extension-installer-' . $product_id; ?>">
    ...
    <div class="bend-content-wrap">
		...
		$brainstrom_bundled_products = (get_option('brainstrom_bundled_products')) ? (array) get_option('brainstrom_bundled_products') : [];
        ...
        <ul class="bsf-extensions-list">
            ...
            <li id="ext-<?php echo $key; ?>" class="bsf-extension <?php echo $class; ?>">
                ...
                <div class="top-section">
                    ...
                    <div class="bsf-extension-info">
						...
						*/
						?>
                        <p class="desc"><?php echo $plugin->description; ?>
                            <span class="author"><cite>By <?php echo $plugin->author; ?></cite></span>
                        </p>
                    </div>
                </div>
                <div class="bottom-section">
					...
					if (!$is_plugin_installed){
						...
						$current_name = strtolower(bsf_get_current_name($plugin->init,
							$plugin->type));
            ...
            <div class="bsf-extensions-no-active">
                <div class="bsf-extensions-title-icon">
                    <span class="dashicons dashicons-smiley"></span></div>
                <p class="bsf-text-light">
                    <em><?php echo __('All available extensions have been installed!',
						'bsf'); ?></em></p>
            </div>
			<?php endif; ?>
        </ul>
        ...
        <ul class="bsf-extensions-list">
            ...
            <li id="ext-<?php echo $key; ?>" class="bsf-extension <?php echo $class; ?>">
                ...
                <div class="bottom-section">
					...
					if (!$is_plugin_installed){
						...
						$current_name = strtolower(bsf_get_current_name($plugin->init,
							$plugin->type));
						$current_name = preg_replace('![^a-z0-9]+!i', '-',
							$current_name);
						if (is_multisite()){
							$link = network_admin_url('plugins.php#' . $current_name);
						}else{
            ...
            <div class="bsf-extensions-no-active">
                <div class="bsf-extensions-title-icon">
                    <span class="dashicons dashicons-download"></span></div>
                <p class="bsf-text-light"><em><?php echo __('No extensions installed yet!',
					'bsf'); ?></em></p>
            </div>
			<?php endif; ?>
        </ul>
        ...
        <div class="bsf-extensions-no-active">
            <div class="bsf-extensions-title-icon">
                <span class="dashicons dashicons-download"></span></div>
            <p class="bsf-text-light"><em><?php echo __('No extensions available yet!',
				'bsf'); ?></em></p>
