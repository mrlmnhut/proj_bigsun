...
if (!function_exists('install_bsf_product')){
	function install_bsf_product($install_id){
		...
		$Plugin_Upgrader->strings['downloading_package'] = __('Downloading package from Server',
			'bsf');
		$Plugin_Upgrader->strings['remove_old']          = __('Removing old plugin, if exists',
			'bsf');
...
if (!function_exists('bsf_install_callback')){
	function bsf_install_callback(){
		...
		$redirect_url         = apply_filters('redirect_after_extension_install',
			$redirect_url = '', $product_id);
