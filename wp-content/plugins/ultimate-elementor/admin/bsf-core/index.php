		...
		if (empty($GLOBALS['admin_page_hooks']['bsf-registration'])){
			...
			if (is_multisite() && is_network_admin()){
				if (defined('BSF_REG_MENU_TO_SETTINGS') && (BSF_REG_MENU_TO_SETTINGS == TRUE || BSF_REG_MENU_TO_SETTINGS == 'true')){
					$page = add_submenu_page('settings.php', BSF_UPDATER_FULLNAME,
						BSF_UPDATER_SHORTNAME, 'administrator', 'bsf-registration',
						'bsf_registration');
				}else{
					$page = add_menu_page(BSF_UPDATER_FULLNAME, BSF_UPDATER_SHORTNAME,
						'administrator', 'bsf-registration', 'bsf_registration', '', $place);
				}
			}elseif (!is_multisite() && !is_network_admin()){
				if (defined('BSF_REG_MENU_TO_SETTINGS') && (BSF_REG_MENU_TO_SETTINGS == TRUE || BSF_REG_MENU_TO_SETTINGS == 'true')){
					$page = add_options_page(BSF_UPDATER_FULLNAME, BSF_UPDATER_SHORTNAME,
						'administrator', 'bsf-registration', 'bsf_registration');
				}else{
					$page = add_dashboard_page(BSF_UPDATER_FULLNAME, BSF_UPDATER_SHORTNAME,
						'administrator', 'bsf-registration', 'bsf_registration');
				}
			}
		}
 ...
 * Checks if theme or plugin is active, if it is not active, the menu for installer should not be
 * registered.
 *
	...
	function register_bsf_extension_page_network(){
		...
		foreach ($themes as $theme){
			if ($theme->Name == 'iMedica'){
				add_submenu_page($parent_slug, __('iMedica Extensions', 'bsf'),
					__('iMedica Extensions', 'bsf'), 'manage_options', 'bsf-extensions-10395942',
					'bsf_extensions_callback');
				break;
			}
		}
		...
		$installer_menu = '';
		$reg_menu       = [];
		$reg_menu       = get_site_option('bsf_installer_menu', []);
...
if (!function_exists('init_bsf_core')){
	...
	function init_bsf_core(){
		...
		$bsf_authors = apply_filters('bsf_authors_list', [
			'Brainstorm Force'
		]);
...
if (!function_exists('register_bsf_core_admin_styles')){
	function register_bsf_core_admin_styles($hook){
		...
		if (in_array($hook, $hook_array) || strpos($hook, 'bsf-extensions') !== FALSE){
			...
			wp_register_script('brainstorm-switch', bsf_core_url('/assets/js/switch.js'),
				['jquery'], BSF_UPDATER_VERSION, TRUE);
			wp_enqueue_script('brainstorm-switch');

			wp_register_script('bsf-core', bsf_core_url('/assets/js/bsf-core.js'), ['jquery'],
				BSF_UPDATER_VERSION, TRUE);
		...
		if (in_array($hook, $hook_frosty_array)){
			wp_register_script('bsf-core-frosty', bsf_core_url('/assets/js/frosty.js'), [],
				BSF_UPDATER_VERSION);
			wp_enqueue_script('bsf-core-frosty');

			wp_register_style('bsf-core-frosty-style', bsf_core_url('/assets/css/frosty.css'), [],
				BSF_UPDATER_VERSION);
...
if (!function_exists('brainstrom_product_id_by_name')){
	...
	function brainstrom_product_id_by_name($product_name){
		...
		foreach ($brainstrom_products as $key => $value){
			foreach ($value as $key => $product){
				if (isset($product['product_name']) && strcasecmp($product['product_name'],
						$product_name) == 0){
...
if (!function_exists('bsf_registration_page_url')){
	...
	function bsf_registration_page_url($append = '', $product_id = ''){
		...
		$bsf_updater_options       = get_option('bsf_updater_options', []);
...
if (!function_exists('bsf_set_options')){
	...
	function bsf_set_options(){
		...
		$skip_author_products = apply_filters('bsf_skip_author_registration', $products = []);
		$ids                  = [];
		...
		$skip_brainstorm_menu_products = apply_filters('bsf_skip_braisntorm_menu',
			$default_skip_brainstorm_menu);
		$ids                           = [];
		$skip_brainstorm_menu          = get_site_option('bsf_skip_braisntorm_menu', FALSE);
		$brainstorm_products           = bsf_get_brainstorm_products(TRUE);
 ...
 *
 * brainstorm_options option saves the data related to all the brainstorm products required for
...
if (!function_exists('bsf_get_brainstorm_products')){
	...
	function bsf_get_brainstorm_products($mix = FALSE){
		$brainstorm_products = get_option('brainstrom_products', []);

		if ($mix == TRUE){
			$plugins = (isset($brainstorm_products['plugins'])) ? $brainstorm_products['plugins'] : [];
			$themes  = (isset($brainstorm_products['themes'])) ? $brainstorm_products['themes'] : [];
