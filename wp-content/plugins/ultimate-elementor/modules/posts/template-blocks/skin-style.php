...
abstract class Skin_Style{
	...
	public function render_featured_title(){
        ...
        <h3 class="uael-post__title">
			...
			<?php if ($this->get_instance_value('link_title')){ ?>
                ...
                <a href="<?php echo apply_filters('uael_single_post_link', get_the_permalink(),
					get_the_ID(), $settings); ?>" target="<?php echo $target; ?>">
					<?php the_title(); ?>
                </a>
	...
	public function render_featured_meta_data(){
		...
		$sequence = apply_filters('uael_post_meta_sequence',
			['author', 'date', 'comments', 'cat', 'tag']);
		?>
	...
	public function render_featured_image(){
		...
		if ('yes' === $this->get_instance_value('link_img')){
			$href   = apply_filters('uael_single_post_link', get_the_permalink(), get_the_ID(),
				$settings);
	...
	public function render_date(){
        ...
        <span class="uael-post__date">
			...
			<?php echo apply_filters('uael_post_the_date_format', get_the_date(), get_the_ID(),
	...
	public function render_comments(){
        ...
        <span class="uael-post__comments">
			<?php if (UAEL_Helper::is_elementor_updated()){ ?><?php if (!empty($icon) || !empty($new_icon)){ ?><?php
	...
	public function get_meta_html_by_prefix($terms, $prefix){
		...
		$link_meta = apply_filters('uael_link_taxomony_badge',
			$this->get_instance_value('link_meta'));
		...
		printf('<span class="uael-post__terms-meta uael-post__terms-meta-%s">%s</span>', $prefix,
			$result);
		...
		if ('yes' === $this->get_instance_value('show_cta')){
			...
			$this->add_render_attribute(
				...
				[
					...
					'href'   => apply_filters('uael_single_post_link', get_the_permalink(),
						get_the_ID(), $settings),
            ...
            <a <?php echo $this->get_render_attribute_string('cta_link' . get_the_ID()); ?>>
					<span class="elementor-button-content-wrapper">
						...
						if (UAEL_Helper::is_elementor_updated()){
							if (!empty($icon) || !empty($new_icon)) :
                                ...
                                <span <?php echo $this->get_render_attribute_string('icon' . get_the_ID()); ?>>
									...
									if ($cta_icon_migrated || $cta_icon_is_empty){
										\Elementor\Icons_Manager::render_icon($new_icon,
											['aria-hidden' => 'true']);
									}else{
										?>
							...
							<?php endif; ?><?php }elseif (!empty($icon)){ ?>
                            <span <?php echo $this->get_render_attribute_string('icon' . get_the_ID()); ?>>
								<i class="<?php echo esc_attr($icon); ?>" aria-hidden="true"></i>
							</span>
						<?php } ?>

						<span class="elementor-button-text"><?php echo apply_filters('uael_post_cta_text',
								$this->get_instance_value('cta_text'), get_the_ID(),
								$settings); ?></span>
					</span>
            </a>
			<?php
	...
	public function render_pagination(){
		...
		if (!in_array($this->get_instance_value('post_structure'),
			['normal', 'featured', 'masonry'], TRUE)){
			return;
		}
		...
		if ($total_pages > 1){
			...
			?>
            <nav class="uael-grid-pagination" <?php echo $class; ?> role="navigation" aria-label="<?php _e('Pagination',
				'uael'); ?>" data-total="<?php echo $total_pages; ?>">
	...
	public function render_filters(){
        ...
        <div class="uael-post__header-filters-wrap<?php echo $tab_responsive; ?>">
            <ul class="uael-post__header-filters">
                <li class="uael-post__header-filter uael-filter__current" data-filter="*"><?php echo ('All' === $all || '' === $all) ? __('All',
						'uael') : $all; ?></li>
			...
			<?php if ('yes' === $this->get_instance_value('tabs_dropdown')){ ?>
                <div class="uael-filters-dropdown">
                    <div class="uael-filters-dropdown-button"><?php echo ('All' === $all || '' === $all) ? __('All',
							'uael') : $all; ?><i class="fa fa-angle-down"></i></div>

                    <ul class="uael-filters-dropdown-list uael-post__header-filters">
                        <li class="uael-filters-dropdown-item uael-post__header-filter uael-filter__current" data-filter="*"><?php echo ('All' === $all || '' === $all) ? __('All',
								'uael') : $all; ?></li>
						<?php foreach ($filters as $key => $value){ ?>
	...
	public function render_search(){
        ...
        <div class="uael-post__grid-empty">
			...
			<?php if ('yes' === $settings['show_search_box']){ ?><?php get_search_form(); ?><?php } ?>
        </div>
		<?php
	}
	...
	public function get_body(){
		...
		if (
			...
			$this->add_render_attribute('outer_wrapper', 'data-default-filter',
				$this->get_instance_value('default_filter'));
		}
		...
		<?php do_action('uael_posts_after_outer_wrap', $settings); ?><?php
	}
