...
class Buttons extends Common_Widget{
	...
	protected function register_buttons_content_controls(){
		...
		$repeater->add_control(
			...
			[
				...
				'raw'             => __('Set custom styles that will only affect this specific button.',
					'uael'),
	...
	protected function register_spacing_content_controls(){
		...
		$this->add_control(
			...
			[
				...
				'description'  => __('Choose on what breakpoint where the buttons will stack.',
					'uael'),
				'type'         => Controls_Manager::SELECT,
				'default'      => 'none',
				'options'      => [
			...
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf(__('%1$s Getting started article » %2$s', 'uael'),
						'<a href="https://uaelementor.com/docs/multi-buttons-widget/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">',
						'</a>'),
			...
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf(__('%1$s Getting started video » %2$s', 'uael'),
						'<a href="https://www.youtube.com/watch?v=Izbr-oO0VkU&list=PL1kzJGWGPrW_7HabOZHb6z88t_S8r-xAc&index=13" target="_blank" rel="noopener">',
						'</a>'),
					'content_classes' => 'uael-editor-doc',
				]
			);
	...
	protected function render_button_text($button, $i){
		...
		$this->add_render_attribute('content-wrapper', 'class',
			'uael-buttons-icon-' . $button['icon_align']);

		$this->add_render_attribute('icon-align_' . $i, 'class',
			'elementor-align-icon-' . $button['icon_align']);
        ...
        <span <?php echo $this->get_render_attribute_string('content-wrapper'); ?>>
			<?php if (UAEL_Helper::is_elementor_updated()){ ?><?php if (!empty($button['icon']) || !empty($button['new_icon'])) : ?>
                <span <?php echo $this->get_render_attribute_string('icon-align_' . $i); ?>>
						...
						if ($is_new || $migrated){
							...
							\Elementor\Icons_Manager::render_icon($button['new_icon'],
								['aria-hidden' => 'true']);
			...
			<?php endif; ?><?php }elseif (!empty($button['icon'])){ ?>
                <span <?php echo $this->get_render_attribute_string('icon-align_' . $i); ?>>
					<i class="<?php echo esc_attr($button['icon']); ?>" aria-hidden="true"></i>
				</span>
			<?php } ?>
        ...
        <div class="uael-dual-button-outer-wrap">
            <div class="uael-dual-button-wrap">
				...
				for ($i = 0; $i < count($settings['buttons']); $i ++) :
					...
					$this->add_render_attribute('button_wrap_' . $i, 'class',
						'uael-button-wrapper elementor-button-wrapper uael-dual-button');
					$this->add_render_attribute('button_wrap_' . $i, 'class',
						'elementor-repeater-item-' . $button['_id']);
					$this->add_render_attribute('button_wrap_' . $i, 'class',
						'uael-dual-button-' . $i);

					$this->add_render_attribute('button_' . $i, 'class',
