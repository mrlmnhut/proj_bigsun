...
if ('icon' === $settings['uael_infobox_image_type'] || 'photo' === $settings['uael_infobox_image_type']){
	...
	$this->add_render_attribute('classname', 'class',
		'uael-imgicon-style-' . $settings['infobox_imgicon_style']);

	if ('above-title' === $settings['infobox_image_position'] || 'below-title' === $settings['infobox_image_position']){
		$this->add_render_attribute('classname', 'class',
			' uael-infobox-' . $settings['infobox_align']);
	}
	...
	if ('icon' === $settings['uael_infobox_image_type']){
		$this->add_render_attribute('classname', 'class',
			' infobox-has-icon uael-infobox-icon-' . $settings['infobox_image_position']);
	}
	if ('photo' === $settings['uael_infobox_image_type']){
		$this->add_render_attribute('classname', 'class',
			' infobox-has-photo uael-infobox-photo-' . $settings['infobox_image_position']);
	}
	if ('above-title' !== $settings['infobox_image_position'] && 'below-title' !== $settings['infobox_image_position']){
	...
	if ('left' === $settings['infobox_overall_align'] || 'center' === $settings['infobox_overall_align'] || 'right' === $settings['infobox_overall_align']){
		...
		$this->add_render_attribute('classname', 'class',
			' uael-infobox-' . $settings['infobox_overall_align']);
	}
}

$this->add_render_attribute('classname', 'class',
	' uael-infobox-link-type-' . $settings['infobox_cta_type']);
...
<div <?php echo $this->get_render_attribute_string('classname'); ?>>
    <div class="uael-infobox-left-right-wrap">
		...
		if ('module' === $settings['infobox_cta_type'] && '' !== $settings['infobox_text_link']){
			?>
            <a href="<?php echo $_link; ?>" target="<?php echo $_target; ?>" <?php UAEL_Helper::get_link_rel($_target,
				$_nofollow, 1); ?> class="uael-infobox-module-link"></a>
			<?php
		}
		?>
