...
class Skin_Style{
	...
	public function render_date($settings){
		...
		echo apply_filters('uael_timeline_the_date_format', get_the_date(),
	...
	public function render_pagination($settings, $query, $query_obj){
		...
		if ($total_pages > 1){
			...
			?>
            <nav class="uael-timeline-pagination" <?php echo $class; ?> role="navigation" aria-label="<?php _e('Pagination',
				'uael'); ?>">
	...
	public function get_body($settings, $query, $query_obj, $dynamic){
		...
		if (!$is_editor){
			if ('yes' === $settings['timeline_infinite']){
				$this->add_render_attribute('timeline_days', 'class',
					'uael-timeline-infinite-load');
			}
		}
		$count        = 0;
		$current_side = '';
        ...
        <div <?php echo $this->get_render_attribute_string('timeline_main'); ?>>
            <div <?php echo $this->get_render_attribute_string('timeline_days'); ?>>
				...
				$this->add_render_attribute('heading_setting_key', 'class',
				...
				$this->add_render_attribute('current_' . $post_id, 'class',
					'elementor-repeater-item-' . $post_id);
				$this->add_render_attribute('current_' . $post_id, 'class',
					'uael-timeline-field animate-border');
				$this->add_render_attribute('current_' . $post_id, 'class', 'out-view');
				$this->add_render_attribute('timeline_alignment' . $post_id, 'class',
					'uael-day-new');

				$this->add_render_attribute('data_alignment' . $post_id, 'class',
				...
				$this->add_render_attribute('timeline_events' . $post_id, 'class',
					'uael-events-new');
				$this->add_render_attribute('timeline_events_inner' . $post_id, 'class',
									...
									if (UAEL_Helper::is_elementor_updated()){
										if (!empty($settings['timeline_all_icon']) || !empty($settings['new_timeline_all_icon'])){
											...
											if ($icon_migrated || $icon_is_new){
												...
												\Elementor\Icons_Manager::render_icon($settings['new_timeline_all_icon'],
													['aria-hidden' => 'true']);
                                        ...
                                        <div class="uael-timeline-date-hide uael-date-inner">
                                            <div class="inner-date-new">
												...
												<?php }elseif ('updated' === $settings['post_timeline_date_type']){ ?>
                                                    <p><?php echo get_the_modified_date('',
															$post_id); ?></p>
												<?php }elseif ('custom' === $settings['post_timeline_date_type']){ ?>
                                                    <p>
														...
														if ('' !== $dynamic_date){
															...
															$custom_meta = apply_filters('uael_timeline_date_content',
																$post_id, $settings);
															echo $custom_meta;
														}
														?>
