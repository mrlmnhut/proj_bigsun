...
<div <?php echo $this->get_render_attribute_string('timeline_wrapper'); ?>>
    ...
    <div <?php echo $this->get_render_attribute_string('timeline_main'); ?>>
        <div <?php echo $this->get_render_attribute_string('timeline_days'); ?>>
			...
			$heading_setting_key = $this->get_repeater_setting_key('timeline_single_heading',
				'timelines', $index);
			...
			$content_setting_key = $this->get_repeater_setting_key('timeline_heading_content',
				'timelines', $index);
			$this->add_render_attribute($content_setting_key, 'class',
				'uael-timeline-desc-content');
			$this->add_inline_editing_attributes($content_setting_key, 'advanced');


			$date_setting_key = $this->get_repeater_setting_key('timeline_single_date', 'timelines',
				$index);
			$this->add_inline_editing_attributes($date_setting_key, 'none');


			if (!empty($item['timeline_single_link']['url'])){
				$this->add_render_attribute('url_' . $item['_id'], 'href',
					$item['timeline_single_link']['url']);
			...
			$this->add_render_attribute('current_' . $item['_id'], 'class',
				'elementor-repeater-item-' . $item['_id']);
			$this->add_render_attribute('current_' . $item['_id'], 'class',
				'uael-timeline-field animate-border');
			$this->add_render_attribute('current_' . $item['_id'], 'class', 'out-view');
			$this->add_render_attribute('timeline_alignment' . $item['_id'], 'class',
				'uael-day-new');

			$this->add_render_attribute('data_alignment' . $item['_id'], 'class',
			...
			$this->add_render_attribute('timeline_events' . $item['_id'], 'class',
				'uael-events-new');
			$this->add_render_attribute('timeline_events_inner' . $item['_id'], 'class',
										...
										if (UAEL_Helper::is_elementor_updated()){
											if (isset($item['timeline_single_icon']) || isset($item['new_timeline_single_icon'])){
												...
												if ($timeline_single_migrated || $timeline_single_is_new){
													\Elementor\Icons_Manager::render_icon($item['new_timeline_single_icon'],
														['aria-hidden' => 'true']);
									...
									if ('' !== $item['timeline_single_date']){
                                        ...
                                        <div class="uael-timeline-date-hide uael-date-inner">
                                            <div class="inner-date-new">
                                                <p <?php echo $this->get_render_attribute_string($date_setting_key); ?>><?php echo $item['timeline_single_date']; ?></p>
                                            </div>
                                        </div>
									<?php } ?>
				...
				<?php if ('center' === $settings['timeline_align']){ ?>
                    <div class="uael-timeline-date-new">
                        <div class="uael-date-new">
                            <div class="inner-date-new">
                                <div <?php echo $this->get_render_attribute_string($date_setting_key); ?>><?php echo $item['timeline_single_date']; ?></div>
                            </div>
                        </div>
                    </div>
				<?php } ?>
