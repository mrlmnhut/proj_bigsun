...
class GoogleMap extends Common_Widget{
		...
		if (parent::is_internal_links() && (!isset($map_options['google_api']) || '' === $map_options['google_api'])){
			...
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf(__('To display customized Google Map without an issue, you need to configure Google Map API key. Please configure API key from <a href="%s" target="_blank" rel="noopener">here</a>.',
						'uael'), $admin_link),
			...
			[
				...
				'fields'      => [
					[
						...
						'description' => sprintf('<a href="https://www.latlong.net/" target="_blank">%1$s</a> %2$s',
							__('Click here', 'uael'),
							__('to find Latitude of your location', 'uael')),
					...
					[
						...
						'description' => sprintf('<a href="https://www.latlong.net/" target="_blank">%1$s</a> %2$s',
							__('Click here', 'uael'),
							__('to find Longitude of your location', 'uael')),
						'type'        => Controls_Manager::TEXT,
						'default'     => '',
						'label_block' => TRUE,
						'dynamic'     => [
					...
					[
						...
						'size_units'  => ['px'],
						'description' => __('Note: If you want to retain the image original size, then set the Marker Size as blank.',
							'uael'),
						'default'     => [
							'size' => 30,
							'unit' => 'px',
	...
	protected function register_layout_controls(){
		...
		$this->add_control(
			...
			[
				...
				'description' => sprintf('<a href="https://mapstyle.withgoogle.com/" target="_blank">%1$s</a> %2$s',
					__('Click here', 'uael'),
					__('to get JSON style code to style your map', 'uael')),
				'type'        => Controls_Manager::TEXTAREA,
				'condition'   => [
					'skin'  => 'custom',
					'type!' => 'satellite',
	...
	protected function register_controls_controls(){
		...
		$this->add_control(
			...
			[
				...
				'description' => __('Generally, the map is center aligned. If you have multiple locations & wish to make your first location as a center point, then switch to moderate mode.',
					'uael'),
			]
		);
		...
		if (parent::is_internal_links()){
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf(__('Enable this to group your markers together if you have many in a close proximity to only display one larger marker on your map.<br> Read %1$s this article %2$s for more information.',
						'uael'),
						'<a href="https://uaelementor.com/docs/what-are-cluster-markers-in-uael/" target="_blank" rel="noopener">',
						'</a>'),
		...
		if (parent::is_internal_links()){
			...
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf(__('%1$s Getting started video » %2$s', 'uael'),
						'<a href="https://www.youtube.com/watch?v=jWzW_oT1iSQ&index=6&list=PL1kzJGWGPrW_7HabOZHb6z88t_S8r-xAc" target="_blank" rel="noopener">',
						'</a>'),
					'content_classes' => 'uael-editor-doc',
				]
			);
			...
			$this->add_control(
				...
				[
					'type'            => Controls_Manager::RAW_HTML,
					/* translators: %1$s doc link */
					'raw'             => sprintf(__('%1$s Google Map localization » %2$s', 'uael'),
						'<a href="https://uaelementor.com/docs/how-to-display-uaels-google-maps-widget-in-your-local-language/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">',
						'</a>'),
					'content_classes' => 'uael-editor-doc',
				]
			);
	...
	protected function get_locations(){
		...
		foreach ($settings['addresses'] as $index => $item){
			...
			$location_object[] = apply_filters('uael_google_map_description',
	...
	protected function render(){
		...
		if ('standard' !== $settings['skin']){
			if ('custom' !== $settings['skin']){
				$this->add_render_attribute('google-map', 'data-predefined-style',
					$settings['skin']);
			}elseif (!empty($settings['map_custom_style'])){
				$this->add_render_attribute('google-map', 'data-custom-style',
	...
	protected function _content_template(){
        ...
        return {                'zoom' : ( '' != settings.zoom.size ) ? settings.zoom.size : 4,                'mapTypeId' : ( '' != settings.type ) ? settings.type : 'roadmap',                'mapTypeControl' : ( 'yes' == settings.type_control ) ? true : false,                'streetViewControl' : ( 'yes' == settings.option_streeview ) ? true : false,                'zoomControl' : ( 'yes' == settings.zoom_control ) ? true : false,                'fullscreenControl' : ( 'yes' == settings.fullscreen_control ) ? true : false,                'gestureHandling' : ( 'yes' == settings.scroll_zoom ) ? true : false            };        }
        ...
        if ( 'none' != item.marker_infowindow ) {                    this_location.push( true );                } else {                    this_location.push( false );                }                this_location.push( item.map_title );                this_location.push( item.map_description );

        if (                    'custom' == item.marker_icon_type &&                    'undefined' != typeof item.marker_icon &&                    '' != item.marker_icon.url                ) {                    this_location.push( 'custom' );                    this_location.push( item.marker_icon.url );                    this_location.push( item.custom_marker_size.size );                } else {                    this_location.push( "" );                    this_location.push( "" );                    this_location.push( "" );                }

        if ( 'load' == item.marker_infowindow ) {                    this_location.push( 'iw_open' );                } else {                    this_location.push( "" );                }
        ...
        var map_options = get_map_options( settings );        var locations    = get_locations( settings );

        view.addRenderAttribute(            'google-map',            {                'class' : 'uael-google-map',                'data-map_options' : JSON.stringify( map_options ),                'data-cluster' : settings.cluster,                'data-max-width' : settings.info_window_size.size,                'data-locations' : JSON.stringify( locations ),                'data-animate'   : settings.animate,                'data-auto-center'   : settings.auto_center,            }        );
