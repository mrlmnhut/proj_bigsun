...
class Team_Member extends Common_Widget{
	...
	protected function render_team_member_content_control(){
		...
		$this->add_control(
			...
			[
				...
				'default'     => __('Use this space to tell a little about your team member. Make it interesting by mentioning his expertise, achievements, interests, hobbies and more.',
					'uael'),
			]
		);
			...
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf(__('%1$s Getting started article » %2$s', 'uael'),
						'<a href="https://uaelementor.com/docs/team-member-widget/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">',
						'</a>'),
	...
	protected function register_style_team_member_image(){
		...
		$this->add_control(
			...
			[
				...
				'description' => __('Choose the breakpoint you want the layout will stack.',
					'uael'),
				'type'        => Controls_Manager::SELECT,
				'default'     => 'none',
				'options'     => [
	...
	protected function render(){
		...
		if ('left' === $settings['image_position'] || 'right' === $settings['image_position']){
			if ('tablet' === $settings['member_mob_view']){
				$this->add_render_attribute('member-classname', 'class',
					'uael-member-stacked-tablet');
			}
			if ('mobile' === $settings['member_mob_view']){
				$this->add_render_attribute('member-classname', 'class',
					'uael-member-stacked-mobile');
			}
			if ('middle' === $settings['member_image_valign']){
				$this->add_render_attribute('member-classname', 'class',
					'uael-member-image-valign-middle');
			}else{
				$this->add_render_attribute('member-classname', 'class',
					'uael-member-image-valign-top');
			}
		}
        ...
        <div <?php echo $this->get_render_attribute_string('member-classname'); ?>>
            <div class="uael-team-member-wrap">
                <div class="uael-member-wrap">
					...
					<?php endif; ?>
					<?php
					$this->add_render_attribute('member_content', 'class',
                    ...
                    <div <?php echo $this->get_render_attribute_string('member_content'); ?>>
						<?php if ('' !== $settings['team_member_name']){ ?>
                            <div class="uael-team-member-name">
								...
								$name_html = sprintf('<%1$s %2$s>%3$s</%1$s>',
									$settings['name_size'],
									$this->get_render_attribute_string('team_member_name'), $name);
								echo $name_html;
								?>
                            </div>
						<?php } ?>
						...
						$this->add_render_attribute('team_member_desig', 'class',
							'uael-team-desig');
									...
									foreach ($settings['social_icon_list'] as $index => $item){
										...
										if (UAEL_Helper::is_elementor_updated()){
											...
											if (!empty($item['social'])){
												$social = str_replace('fa fa-', '',
													$item['social']);
											}

											if (($is_new || $migrated) && 'svg' !== $item['new_social']['library']){
												$social = explode(' ', $item['new_social']['value'],
													2);
												if (empty($social[1])){
													$social = '';
												}else{
											...
											if ($item['link']['is_external']){
												$this->add_render_attribute($link_key, 'target',
													'_blank');
											}
											if ($item['link']['nofollow']){
												$this->add_render_attribute($link_key, 'rel',
													'nofollow');
											}
	...
	protected function _content_template(){
		?>
        <#            if ( 'left' == settings.image_position || 'right' == settings.image_position ) {                if ( 'tablet' == settings.member_mob_view  ) {

        view.addRenderAttribute( 'member-classname', 'class', 'uael-member-stacked-tablet' );                }                if ( 'mobile' == settings.member_mob_view  ) {

        view.addRenderAttribute( 'member-classname', 'class', 'uael-member-stacked-mobile' );                }                if ( 'middle' == settings.member_image_valign ) {                    view.addRenderAttribute( 'member-classname', 'class', 'uael-member-image-valign-middle' );                } else {                    view.addRenderAttribute( 'member-classname', 'class', 'uael-member-image-valign-top' );                }            }            view.addRenderAttribute( 'member-classname', 'class', 'uael-team-member' );        #>
        ...
        <div class="uael-team-member-wrap">
            <div class="uael-member-wrap">
                ...
                #>
                <div class="uael-team-member-image">
                    <img src="{{{ image_url }}}" class="elementor-animation-{{{settings.hover_animation}}}">
                </div>
                <#
                }
                ...
                <div class="uael-team-member-name">
                    ...
                    #>
                    <{{{settings.name_size}}} {{{ view.getRenderAttributeString( 'team_member_name' )}}}>{{{settings.team_member_name}}}
                </
                {{{settings.name_size}}}>
                <# } #>
            </div>
            ...
            <div class="uael-team-member-designation">
                ...
                #>
                <div {{{ view.getRenderAttributeString(
                'team_member_desig' ) }}}>{{{ memberDesignHtml }}}
            </div>
            <# } #>
        </div>                        <# if( 'yes' == settings.separator_settings) {                            if( 'below_desig' == settings.separator_position ) { #>
        <div class="uael-separator-wrapper">
            <div class="uael-separator"></div>
        </div>                            <# } #>                        <# } #>
        <div class="uael-team-member-desc">
            ...
            #>
            <div {{{ view.getRenderAttributeString(
            'team_member_desc' ) }}}>{{{ memberDescHtml }}}
        </div>                            <# } #>                        </div>                        <# if( 'yes' == settings.separator_settings) {                            if( 'below_desc' == settings.separator_position ) { #>
        <div class="uael-separator-wrapper">
            <div class="uael-separator"></div>
        </div>                            <# } #>                        <# } #>                        <# if( 'yes' == settings.social_icons_settings) { #>                            <# var iconsHTML = {}; #>
        <div class="elementor-social-icons-wrapper">
            <div class="uael-team-social-icon">
                <# _.each( settings.social_icon_list, function( item, index ) { #>
				<?php if (UAEL_Helper::is_elementor_updated()){ ?>
                    <# var link = item.link ? item.link.url : '',                                                migrated = elementor.helpers.isIconMigrated( item, 'new_social' ),                                                social = elementor.helpers.getSocialNetworkNameFromIcon( item.new_social, item.social, false, migrated );                                                #>
				...
				<?php }else{ ?>
                    <# var link = item.link ? item.link.url : '',                                            social = item.social.replace( 'fa fa-', '' ); #>
