		...
		if ('file' === $settings['source']){
            ...
            <table <?php echo $this->get_render_attribute_string('uael_table_id'); ?>>
				...
				if ($row_count_h > 1){
					...
					// TH.
					if (TRUE === $first_row_h){
						$this->add_render_attribute('current_' . $head['_id'], 'data-sort',
							$cell_counter_h);
					}
					$this->add_render_attribute('current_' . $head['_id'], 'class', 'sort-this');
					$this->add_render_attribute('current_' . $head['_id'], 'class',
						'elementor-repeater-item-' . $head['_id']);
					$this->add_render_attribute('current_' . $head['_id'], 'class',
						'uael-table-col');
					if (1 < $head['heading_col_span']){
						$this->add_render_attribute('current_' . $head['_id'], 'colspan',
							$head['heading_col_span']);
					}
					if (1 < $head['heading_row_span']){
						$this->add_render_attribute('current_' . $head['_id'], 'rowspan',
							$head['heading_row_span']);
					}
					// Sort Icon.
					if ('yes' === $settings['sortable'] && TRUE === $first_row_h){
						$this->add_render_attribute('icon_sort_' . $head['_id'], 'class',
							'uael-sort-icon');
					}
					if ($head['head_image']['url']){
						$this->add_render_attribute('uael_head_col_img' . $head['_id'], 'src',
							$head['head_image']['url']);
						$this->add_render_attribute('uael_head_col_img' . $head['_id'], 'class',
							'uael-col-img--' . $settings['all_image_align']);
						$this->add_render_attribute('uael_head_col_img' . $head['_id'], 'title',
							get_the_title($head['head_image']['id']));
						$this->add_render_attribute('uael_head_col_img' . $head['_id'], 'alt',
							Control_Media::get_image_alt($head['head_image']));
					}
					// ICON.
					$this->add_render_attribute('uael_heading_icon_align' . $head['_id'], 'class',
						'uael-align-icon--' . $settings['all_icon_align']);
					...
					if ('cell' === $head['header_content_type']) {
                        ...
                        <th <?php echo $this->get_render_attribute_string('current_' . $head['_id']); ?> scope="col">
							...
							<span <?php echo $this->get_render_attribute_string('uael_table__text'); ?>>
								<?php if ('icon' === $head['header_content_icon_image'] && $head['new_heading_icon']['value']){ ?><?php if ('left' === $settings['all_icon_align']){ ?><?php $this->render_heading_icon($head); ?><?php } ?><?php }else{ ?><?php if ($head['head_image']['url']){ ?><?php if ('left' === $settings['all_image_align']){ ?>
								...
								<?php if ('icon' === $head['header_content_icon_image'] && $head['new_heading_icon']['value']){ ?>
									...
									<?php if ('right' === $settings['all_icon_align']){ ?><?php $this->render_heading_icon($head); ?><?php } ?>

								<?php }else{ ?><?php if ($head['head_image']['url']){ ?><?php if ('right' === $settings['all_image_align']){ ?>
                                    <img <?php echo $this->get_render_attribute_string('uael_head_col_img' . $head['_id']); ?>>
								<?php } ?><?php } ?><?php } ?>
							</span>
				...
				$repeater_cell_text = $this->get_repeater_setting_key('cell_text', 'table_content',
					$cell_inline_count);
				...
				$this->add_render_attribute('uael_cell_icon_align' . $row['_id'], 'class',
					'uael-align-icon--' . $settings['all_icon_align']);

				$this->add_render_attribute('uael_table_col' . $row['_id'], 'class',
					'uael-table-col');
				$this->add_render_attribute('uael_table_col' . $row['_id'], 'class',
					'elementor-repeater-item-' . $row['_id']);
				if (1 < $row['cell_span']){
					$this->add_render_attribute('uael_table_col' . $row['_id'], 'colspan',
						$row['cell_span']);
				}
				if (1 < $row['cell_row_span']){
					$this->add_render_attribute('uael_table_col' . $row['_id'], 'rowspan',
						$row['cell_row_span']);
				}
				if ($row['image']['url']){
					$this->add_render_attribute('uael_col_img' . $row['_id'], 'src',
						$row['image']['url']);
					$this->add_render_attribute('uael_col_img' . $row['_id'], 'class',
						'uael-col-img--' . $settings['all_image_align']);
					$this->add_render_attribute('uael_col_img' . $row['_id'], 'title',
						get_the_title($row['image']['id']));
					$this->add_render_attribute('uael_col_img' . $row['_id'], 'alt',
						Control_Media::get_image_alt($row['image']));
				}
				if (!empty($row['link']['url'])){
					$this->add_render_attribute('col-link-' . $row['_id'], 'href',
						$row['link']['url']);
				...
				if ('cell' === $row['content_type']) {
					...
					if (isset($header_text[$cell_counter]) && $header_text[$cell_counter]){
						$this->add_render_attribute('uael_table_col' . $row['_id'], 'data-title',
					...
					<?php } ?>
                    <span <?php echo $this->get_render_attribute_string('uael_table__text'); ?>>
									<?php if ('icon' === $row['cell_content_icon_image'] && $row['new_cell_icon']['value']){ ?><?php if ('left' === $settings['all_icon_align']){ ?><?php $this->render_row_icon($row); ?><?php } ?><?php }else{ ?><?php if ($row['image']['url']){ ?><?php if ('left' === $settings['all_image_align']){ ?>
									...
									<?php if ('icon' === $row['cell_content_icon_image'] && $row['new_cell_icon']['value']){ ?><?php if ('right' === $settings['all_icon_align']){ ?><?php $this->render_row_icon($row); ?><?php } ?><?php }else{ ?><?php if ($row['image']['url']){ ?><?php if ('right' === $settings['all_image_align']){ ?>
                                        <img <?php echo $this->get_render_attribute_string('uael_col_img' . $row['_id']); ?>>
									<?php } ?><?php } ?><?php } ?>
								</span>
					<?php if (!empty($row['link']['url'])){ ?>
