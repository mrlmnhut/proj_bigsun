    ...
    <div class="uael-review-wrap">
        <div class="uael-review uael-review-type-<?php echo $review['source']; ?>">
            ...
            <div class="uael-review-inner-wrap">
				<?php if ('yes' === $this->get_instance_value('review_content')){ ?><?php
					...
					if ('' !== $this->get_instance_value('review_content_length')){
						...
						if (count($words) > $content_length){
							array_pop($words);
							$the_content = implode(' ',
								$words); // put in content only the number of word that is set in $content_length.
							$the_content .= '...';
					...
					<?php $this->get_reviews_header($review, $photolink, $settings); ?><?php } ?>
            </div>
        </div>
    </div>
<?php
