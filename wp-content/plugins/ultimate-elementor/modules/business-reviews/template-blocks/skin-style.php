...
<g>
	...
	public function get_google_api($settings){
		...
		if ('' !== $integration_options['google_places_api']){
			...
			if ('yes' === self::$google_api_status){
				...
				echo __('<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Google Error Message:</span> Incorrect Google API key! Please set up the API key from UAE settings.</span>',
					'uael');

				return FALSE;
			}
		}elseif ($is_editor){
			echo __('<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Google Error Message:</span> Please configure the Google API key to display the reviews.</span>',
				'uael');
		...
		if (FALSE === $result){
			...
			if (is_wp_error($result)){
				...
				if ($is_editor){
					...
					echo sprintf(__('<span class="uael-reviews-notice-message">Something went wrong while fetching Google reviews: %1$s</span>',
						'uael'), $error_message);
				}
			...
			switch ($result_status){
				case 'OVER_QUERY_LIMIT':
					...
					echo sprintf(__('<span class="uael-reviews-notice-message elementor-clickable"><span class="uael-reviews-error-message">Google Error Message: </span>OVER_QUERY_LIMIT</br>You have exceeded your daily request quota for this API. If you did not set a custom daily request quota, verify your project has an active billing account. Visit your %1$s Google API console %2$s to activate billing. </span>',
						'uael'), '<a href="http://g.co/dev/maps-no-account">', '</a>');
				...
				case 'REQUEST_DENIED':
					echo __('<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Google Error Message: </span>REQUEST_DENIED</br>Invalid Google API key! Please verify your API key from UAE settings.</span>',
						'uael');
					delete_transient($transient_name);
				...
				case 'UNKNOWN_ERROR':
					echo __('<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Google Error Message: </span>UNKNOWN_ERROR </br>Seems like a server-side error; Please try again later.</span>',
						'uael');
					delete_transient($transient_name);
				...
				case 'INVALID_REQUEST':
					echo __('<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Google Error Message: </span>INVALID_REQUEST </br>Please check if the entered Place ID is invalid.</span>',
						'uael');
					delete_transient($transient_name);
				...
				case 'OK':
					if (!property_exists($result->result, 'reviews')){
						echo __('<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Google Error Message:</span> It seems like the Google place you have selected does not have any reviews.</span>',
							'uael');
						delete_transient($transient_name);
	...
	public function get_yelp_api($settings){
		...
		if ('' !== $business_id){
			...
			if ('' !== $integration_options['yelp_api']){
				...
				echo __('<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Yelp Error Message:</span> Please set the Yelp API key to display the reviews.</span>',
					'uael');
			...
			if (is_wp_error($result)){
				...
				if ($is_editor){
					/* translators: %1$s doc link */
					echo sprintf(__('<span class="uael-reviews-notice-message">Something went wrong: %1$s</span>',
						'uael'), $error_message);
				}
				delete_transient($transient_name);
			...
			if (200 !== $response_code && !empty($response_message)){
				if ($is_editor){
					...
					if ('VALIDATION_ERROR' === $error_code){
						echo __('<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Yelp Error Message:</span> Incorrect Yelp API key. Please set up the API key from UAE settings.</span>',
							'uael');
					}elseif ('BUSINESS_NOT_FOUND' === $error_code){
						echo __('<span class="uael-reviews-notice-message"><span class="uael-reviews-error-message">Yelp Error Message:</span> Incorrect Business ID.</span>',
							'uael');
					}
				}
				delete_transient($transient_name);
				...
				if ($is_editor){
					/* translators: %1$s doc link */
					echo sprintf(__('<span class="uael-reviews-notice-message"> %1$s Unknown error occurred.</span>',
						'uael'), $response_code);
				}
				delete_transient($transient_name);
	...
	public function get_reviewer_name($review, $settings){
		if ('yes' === $this->get_instance_value('reviewer_name')){ ?><?php if ('yes' === $this->get_instance_value('reviewer_name_link')){ ?>
...
<g>
            ...
            <div class="uael-review-details">
				...
				<?php if ('yes' === $this->get_instance_value('review_rating')){ ?>
                    <span class="elementor-star-rating__wrapper">
						<span class="uael-star-rating"><?php echo $this->render_stars($total_rating,
								$review, $settings); ?></span>
					</span>
				<?php } ?>
				<?php
				if ('yes' === $this->get_instance_value('review_date')){
        ...
        <div class="uael-business-reviews-widget">
            <div <?php echo $this->get_render_attribute_string('uael-reviews-parent-data'); ?> <?php echo $this->get_carousel_attr(); ?>>
				...
				if ('rating' === $settings['reviews_filter_by']){
					usort($reviews, [$this, 'filter_by_rating']);
				}elseif ('date' === $settings['reviews_filter_by']){
					usort($reviews, [$this, 'filter_by_date']);
				}
	 ...
	 * `$this->add_render_attribute( 'button', [ 'class' => 'custom-button-class', 'id' =>
	 * 'custom-button-id' ] );`
	 *
	...
	public function add_render_attribute($element, $key = NULL, $value = NULL, $overwrite = FALSE){
		...
		if ($overwrite){
			...
			$this->_render_attributes[$element][$key] = array_merge($this->_render_attributes[$element][$key],
				$value);
		}
	...
	public function get_render_attribute_string($element){
		...
		foreach ($render_attributes as $attribute_key => $attribute_values){
			$attributes[] = sprintf('%1$s="%2$s"', $attribute_key,
				esc_attr(implode(' ', $attribute_values)));
		}
