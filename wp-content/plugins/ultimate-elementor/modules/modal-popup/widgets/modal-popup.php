...
class Modal_Popup extends Common_Widget{
	...
	protected function register_general_content_controls(){
		...
		$this->add_control(
			...
			[
				...
				'label_off'    => __('No', 'uael'),
				'label_on'     => __('Yes', 'uael'),
				'description'  => __('Enabling this option will reduce the page size and page loading time. The related CSS and JS scripts will load on request. A loader will appear during loading of the Iframe.',
					'uael'),
				'condition'    => [
					'content_type' => 'iframe',
				],
		...
		$this->add_control(
			...
			[
				...
				'raw'       => sprintf('<p style="font-size: 11px;font-style: italic;line-height: 1.4;color: #a4afb7;">%s</p>',
					__('Append the "?uael-modal-action=modal-popup-id" at the end of your URL.',
						'uael')),
				'condition' => [
					'modal_on' => 'via_url',
				],
	...
	protected function register_content_style_controls(){
		...
		if (UAEL_Helper::is_elementor_updated()){
			...
			$this->add_control(
				...
				[
					...
					'description'      => __('Note: The Upload SVG option is not supported for the Vimeo play icon.',
						'uael'),
					'type'             => Controls_Manager::ICONS,
		...
		if (UAEL_Helper::is_elementor_updated()){
			...
			$this->add_control(
				...
				[
					'label'            => __('Select Icon', 'uael'),
					'description'      => __('Note: The Upload SVG option is not supported for the YouTube play icon.',
						'uael'),
					'type'             => Controls_Manager::ICONS,
		...
		if (parent::is_internal_links()){
			...
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf(__('%1$s Getting started video » %2$s', 'uael'),
						'<a href="https://www.youtube.com/watch?v=kXuXfaetch4&list=PL1kzJGWGPrW_7HabOZHb6z88t_S8r-xAc&index=14" target="_blank" rel="noopener">',
						'</a>'),
					'content_classes' => 'uael-editor-doc',
				]
			);
			...
			$this->add_control(
				...
				[
					'type'            => Controls_Manager::RAW_HTML,
					/* translators: %1$s doc link */
					'raw'             => sprintf(__('%1$s Trigger Modal Popup on the click of menu » %2$s',
						'uael'),
						'<a href="https://uaelementor.com/docs/how-to-trigger-a-modal-popup-on-the-click-of-a-menu-element/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">',
						'</a>'),
					'content_classes' => 'uael-editor-doc',
				]
			);
			...
			$this->add_control(
				...
				[
					'type'            => Controls_Manager::RAW_HTML,
					/* translators: %1$s doc link */
					'raw'             => sprintf(__('%1$s Close Modal Popup on click of button or link » %2$s',
						'uael'),
						'<a href="https://uaelementor.com/docs/various-options-to-close-a-modal-popup-in-uael/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">',
						'</a>'),
					'content_classes' => 'uael-editor-doc',
				]
			);
			...
			$this->add_control(
				...
				[
					'type'            => Controls_Manager::RAW_HTML,
					/* translators: %1$s doc link */
					'raw'             => sprintf(__('%1$s Trigger Modal Popup from another widget » %2$s',
						'uael'),
						'<a href="https://uaelementor.com/docs/how-to-open-a-modal-popup-from-another-widget/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">',
						'</a>'),
					'content_classes' => 'uael-editor-doc',
				]
			);
			...
			$this->add_control(
				...
				[
					'type'            => Controls_Manager::RAW_HTML,
					/* translators: %1$s doc link */
					'raw'             => sprintf(__('%1$s Modal Popup JS Triggers » %2$s', 'uael'),
						'<a href="https://uaelementor.com/docs/modal-popup-js-triggers/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">',
						'</a>'),
					'content_classes' => 'uael-editor-doc',
				]
			);
	...
	public function get_modal_content($settings, $node_id){
		...
		switch ($content_type){
			...
			case 'iframe':
				...
				break;
			case 'saved_rows':
				return \Elementor\Plugin::$instance->frontend->get_builder_content_for_display(apply_filters('wpml_object_id',
					$settings['ct_saved_rows'], 'page'));
			case 'saved_modules':
	...
	public function render_button($node_id, $settings){
		...
		$this->add_render_attribute('button', 'class',
			'uael-trigger elementor-button-link elementor-button elementor-clickable');
		...
		if (!empty($settings['btn_align'])){
			$this->add_render_attribute('wrapper', 'class',
				'elementor-align-' . $settings['btn_align']);
			$this->add_render_attribute('wrapper', 'class',
				'elementor-tablet-align-' . $settings['btn_align_tablet']);
			$this->add_render_attribute('wrapper', 'class',
				'elementor-mobile-align-' . $settings['btn_align_mobile']);
		}

		if ($settings['btn_hover_animation']){
			$this->add_render_attribute('button', 'class',
				'elementor-animation-' . $settings['btn_hover_animation']);
		}
				...
				?><?php if (!empty($settings['btn_icon']) || !empty($settings['new_btn_icon'])) : ?>
                    <span <?php echo $this->get_render_attribute_string('icon-align'); ?>>
						...
						if ($button_is_new || $button_migrated){
							\Elementor\Icons_Manager::render_icon($settings['new_btn_icon'],
								['aria-hidden' => 'true']);
		...
		if ('icon' === $settings['close_source']){
			if (UAEL_Helper::is_elementor_updated()){
				...
				if ($close_is_new || $close_migrated){
					\Elementor\Icons_Manager::render_icon($settings['new_close_icon'],
						['aria-hidden' => 'true']);
	...
	protected function render_action_html(){
		...
		if ('button' === $settings['modal_on']){
            ...
            <div class="uael-builder-msg" style="text-align: center;">
                ...
                <p><?php _e('Click here to edit the "Modal Popup" settings. This text will not be visible on frontend.',
						'uael'); ?></p>
            </div>
			<?php
			...
			switch ($settings['modal_on']){
				...
				case 'icon':
					$this->add_render_attribute('action-wrap', 'class',
						'uael-modal-icon-wrap uael-modal-icon');
