	...
	class Price_List extends Common_Widget{
				...
				[
					...
					'default'     => [
						[
							...
							'item_description' => __('I am item content. Click edit button to change this text. Lorem ipsum dolor sit amet, consectetur adipiscing elit.',
								'uael'),
							'price'            => '$20',
							'link'             => ['url' => '#'],
						],
		...
		protected function render_structure_control(){
			...
			$this->add_control(
				...
				[
					...
					'description'  => __('Choose on what breakpoint where the list will stack.',
						'uael'),
					'type'         => Controls_Manager::SELECT,
					'default'      => 'mobile',
					'options'      => [
			...
			if (parent::is_internal_links()){
				...
				$this->add_control(
					...
					[
						...
						'raw'             => sprintf(__('%1$s Getting started article » %2$s',
							'uael'),
							'<a href="https://uaelementor.com/docs/price-list-widget/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">',
							'</a>'),
				...
				$this->add_control(
					...
					[
						'type'            => Controls_Manager::RAW_HTML,
						/* translators: %1$s doc link */
						'raw'             => sprintf(__('%1$s Getting started video » %2$s',
							'uael'),
							'<a href="https://www.youtube.com/watch?v=bVfJNMZOsv0&list=PL1kzJGWGPrW_7HabOZHb6z88t_S8r-xAc&index=13" target="_blank" rel="noopener">',
							'</a>'),
						'content_classes' => 'uael-editor-doc',
					]
				);
				...
				$this->add_control(
					...
					[
						'type'            => Controls_Manager::RAW_HTML,
						/* translators: %1$s doc link */
						'raw'             => sprintf(__('%1$s Minimum height » %2$s', 'uael'),
							'<a href="https://uaelementor.com/docs/minimum-height-option-in-price-list/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">',
							'</a>'),
						'content_classes' => 'uael-editor-doc',
					]
				);
				...
				$this->add_control(
					...
					[
						'type'            => Controls_Manager::RAW_HTML,
						/* translators: %1$s doc link */
						'raw'             => sprintf(__('%1$s How to set different image positions? » %2$s',
							'uael'),
							'<a href="https://uaelementor.com/docs/how-to-set-different-image-positions-in-price-list/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">',
							'</a>'),
						'content_classes' => 'uael-editor-doc',
					]
				);
				...
				$this->add_control(
					...
					[
						'type'            => Controls_Manager::RAW_HTML,
						/* translators: %1$s doc link */
						'raw'             => sprintf(__('%1$s How to set hover colors & animations? » %2$s',
							'uael'),
							'<a href="https://uaelementor.com/docs/how-to-set-hover-colors-and-animations-for-price-list/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">',
							'</a>'),
						'content_classes' => 'uael-editor-doc',
					]
				);
		...
		public function render_image($item, $instance){
			...
			if ('custom' === $image_size){
				$image_src = Group_Control_Image_Size::get_attachment_image_src($image_id,
					'image_size', $instance);
			}else{
			...
			return sprintf('<img class="%s" src="%s" alt="%s" />', $class, $image_src,
				$item['title']);
		}
		...
		public function get_price($index, $pos){
			...
			$this->add_render_attribute($price_key, 'class',
				'uael-price-list-price ' . $price_item_cls);
			$this->add_inline_editing_attributes($price_key, 'basic');

			$discount_price_key = $this->get_repeater_setting_key('discount_price', 'price_list',
				$index);

			$this->add_render_attribute($discount_price_key, 'class',
