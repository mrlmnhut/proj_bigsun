...
class Client implements ClientInterface{
	...
	private function buildUri($uri, array $config){
		...
		if ($uri->getHost() && isset($config['idn_conversion']) && ($config['idn_conversion'] !== FALSE)){
			...
			if ($asciiHost === FALSE){
				...
				$errorConstants = array_filter(array_keys(get_defined_constants()),
					function ($name){
	...
	private function applyOptions(RequestInterface $request, array &$options){
		...
		if (isset($options['form_params'])){
			...
			$options['_conditional']                 = Psr7\_caseless_remove(['Content-Type'],
		...
		if (!empty($options['decode_content'])
			...
			// Ensure that we don't have the header in different case and set the new value.
			$options['_conditional']                  = Psr7\_caseless_remove(['Accept-Encoding'],
				$options['_conditional']);
		...
		if (!empty($options['auth']) && is_array($options['auth'])){
			...
			switch ($type){
				case 'basic':
					// Ensure that we don't have the header in different case and set the new value.
					$modify['set_headers']                  = Psr7\_caseless_remove(['Authorization'],
						$modify['set_headers']);
