...
class Google_Client{
	 ...
	 *
	 * @param string|array $scope The scope is expressed as an array or list of space-delimited
	 *     strings.
	 *
	 * @return string
	 */
	 ...
	 *
	 * @param $prompt string
	 *  {@code "none"} Do not display any authentication or consent screens. Must not be specified
	 *     with other values.
	 ...
	 *
	 * @param string|array|null $token The token (access token or a refresh token) that should be
	 *     revoked.
	 *
	...
	private function createApplicationDefaultCredentials(){
		...
		if ($signingKey){
			...
			$credentials               = CredentialsLoader::makeCredentials($scopes,
				$serviceAccountCredentials);
		}else{
