 ...
 *
 * Note that loading an X.509 certificate and resaving it may invalidate the signature.  The reason
 * being that the signature is based on a portion of the certificate that contains optional
 * parameters with default values.  ie. if the parameter isn't there the default value is used.
 * Problem is, if the parameter is there and it just so happens to have the default value there are
 * two ways that that parameter can be encoded.  It can be encoded explicitly or left out all
 * together.  This would effect the signature value and thus may invalidate the the certificate all
...
class X509{
	 ...
	 *
	 * There's no guarantee \phpseclib\File\X509 is going to re-encode an X.509 cert in the same
	 * way it was originally encoded so we take save the portion of the original cert that the
	...
	function __construct(){
        ...
        */
		...
		$this->CRLReason = ['type'    => ASN1::TYPE_ENUMERATED,
		                    'mapping' => [
			                    'unspecified',
			                    'keyCompromise',
			                    'cACompromise',
		...
		$this->IssuingDistributionPoint = ['type'     => ASN1::TYPE_SEQUENCE,
		                                   'children' => [
			                                   'distributionPoint'          => [
				                                                                   'constant' => 0,
				                                                                   'optional' => TRUE,
	...
	function loadX509($cert, $mode = self::FORMAT_AUTO_DETECT){
		...
		$key = $this->_reformatKey($x509['tbsCertificate']['subjectPublicKeyInfo']['algorithm']['algorithm'],
			$key);
				...
				if (is_array($attributes[$i]['value'])){
					...
					for ($j = 0; $j < count($values); $j ++){
						...
						if (!is_bool($map)){
							...
							if ($id == 'pkcs-9-at-extensionRequest' && $this->_isSubArrayValid($values,
									$j)){
					...
					switch ($key){
						...
						case 'iPAddress':
							...
							if (preg_match('#(?:\d{1-3}\.){4}#',
									$components['host'] . '.') && preg_match('#^' . $value . '$#',
									$components['host'])){
								return TRUE;
							}
					}
	 ...
	 *
	 * By default returns false for self-signed certs. Call validateSignature(false) to make this
	 * support self-signed.
	 *
	 * The behavior of this function is inspired by
	 * {@link http://php.net/openssl-verify openssl_verify}.
	 *
				...
				if (!empty($this->CAs)){
					for ($i = 0; $i < count($this->CAs); $i ++){
						...
						switch (TRUE){
							...
							case defined('FILE_X509_IGNORE_TYPE') && $this->getDN(self::DN_STRING,
									$this->currentCert['tbsCertificate']['issuer']) === $this->getDN(self::DN_STRING,
				...
				if (!empty($this->CAs)){
					for ($i = 0; $i < count($this->CAs); $i ++){
						...
						switch (TRUE){
							...
							case defined('FILE_X509_IGNORE_TYPE') && $this->getDN(self::DN_STRING,
									$this->currentCert['tbsCertList']['issuer']) === $this->getDN(self::DN_STRING,
									$ca['tbsCertificate']['subject']):
								$authorityKey = $this->getExtension('id-ce-authorityKeyIdentifier');
								$subjectKeyID = $this->getExtension('id-ce-subjectKeyIdentifier',
									$ca);
	...
	function setDN($dn, $merge = FALSE, $type = 'utf8String'){
		...
		$results = preg_split('#((?:^|, *|/)(?:C=|O=|OU=|CN=|L=|ST=|SN=|postalCode=|streetAddress=|emailAddress=|serialNumber=|organizationalUnitName=|title=|description=|role=|x500UniqueIdentifier=|postalAddress=))#',
	...
	function getDN($format = self::DN_ARRAY, $dn = NULL){
		...
		foreach ($dn['rdnSequence'] as $field){
			...
			if (is_array($value)){
				...
				$value    = strtoupper(preg_replace_callback('#[^\x20-\x7E]#', $callback,
					$value->element));
			}
			$output        .= $desc . '=' . $value;
		...
		switch (TRUE){
			...
			case isset($this->currentCert['tbsCertificate']):
				return $this->getDNProp($propName, $this->currentCert['tbsCertificate']['issuer'],
					$withType);
			case isset($this->currentCert['tbsCertList']):
				return $this->getDNProp($propName, $this->currentCert['tbsCertList']['issuer'],
					$withType);
		}
	...
	function getSubjectDNProp($propName, $withType = FALSE){
		switch (TRUE){
			...
			case !isset($this->currentCert) || !is_array($this->currentCert):
				break;
			case isset($this->currentCert['tbsCertificate']):
				return $this->getDNProp($propName, $this->currentCert['tbsCertificate']['subject'],
					$withType);
			case isset($this->currentCert['certificationRequestInfo']):
				return $this->getDNProp($propName,
					$this->currentCert['certificationRequestInfo']['subject'], $withType);
		}
	 ...
	 *
	 * This is used by the id-ce-authorityKeyIdentifier and the id-ce-subjectKeyIdentifier
	 * extensions.
	 *
	 * @param string $value
	 *
	 ...
	 * @return bool
	 */
	function setRevokedCertificateExtension(
		$serial,
		$id,
		$value,
		$critical = FALSE,
	 ...
	 *
	 * What's returned in the associative array returned by loadX509() (or load*()) is either a
	 * name or an OID if no OID to name mapping is available. The problem with this is that what
	 * may be an unmapped OID in one version of phpseclib may not be unmapped in the next version,
	 * so apps that are looking at this OID may not be able to work from version to version.
	 *
	 * This method will return the OID if a name is passed to it and if no mapping is avialable
	 * it'll assume that what's being passed to it already is an OID and return that instead. A few
	 * examples.
	 *
