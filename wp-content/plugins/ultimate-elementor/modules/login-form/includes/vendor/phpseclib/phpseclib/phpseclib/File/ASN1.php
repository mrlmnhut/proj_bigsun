 ...
 * DER blobs.
 *
 * \phpseclib\File\ASN1 decodes and encodes DER formatted messages and places them in a semantic
 * context.
 *
...
class ASN1{
		...
		switch ($tag){
			...
			case self::TYPE_SET:
				...
				while ($content_pos < $content_len){
					...
					if (!isset($current['headerlength']) && substr($content, $content_pos,
							2) == "\0\0"){
	 ...
	 * ASN.1 Map
	 *
	 * Provides an ASN.1 semantic mapping ($mapping) from a parsed BER-encoding to a human readable
	 * format.
	 *
		...
		switch (TRUE){
			case $mapping['type'] == self::TYPE_ANY:
				...
				if (isset($decoded['constant']) || !isset($this->ANYmap[$intype]) || (ord($this->encoded[$decoded['start']]) & 0x20)){
					return new Element(substr($this->encoded, $decoded['start'],
						$decoded['length']));
				}
						...
						if ($child['type'] != self::TYPE_CHOICE){
							...
							if (isset($constant) && isset($temp['constant'])){
								...
								$maymatch = !isset($child['constant']) && array_search($child['type'],
										[$temp['type'], self::TYPE_ANY, self::TYPE_CHOICE]) !== FALSE;
							}
						}
					}
				...
				if (isset($mapping['mapping'])){
					...
					/*
					   ...
					   "When a "NamedBitList" is used in defining a bitstring type ASN.1 encoding rules are free to add (or remove)
						...
						0 bits."
					*/
					$bits = count($mapping['mapping']) == $size ? [] : array_fill(0,
	 ...
	 * ASN.1 Encode
	 *
	 * DER-encodes an ASN.1 semantic mapping ($mapping).  Some libraries would probably call this
	 * function an ASN.1 compiler.
	 *
	...
	function _encode_der($source, $mapping, $idx = NULL, $special = []){
		...
		switch ($tag){
			...
			case self::TYPE_ANY:
				...
				switch (TRUE){
					case !isset($source):
						return $this->_encode_der(NULL, ['type' => self::TYPE_NULL] + $mapping,
							NULL, $special);
					case is_int($source):
					...
					case is_float($source):
						return $this->_encode_der($source, ['type' => self::TYPE_REAL] + $mapping,
							NULL, $special);
					case is_bool($source):
						return $this->_encode_der($source,
			...
			case self::TYPE_BOOLEAN:
				...
				break;
			default:
				user_error('Mapping provides no type definition for ' . implode('/',
						$this->location));
