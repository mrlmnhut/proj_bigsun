 ...
 * {@link http://en.wikipedia.org/wiki/ANSI_escape_code ANSI escape codes} back. They'd look like
 * chr(0x1B) . '[00m' or whatever (0x1B = ESC).  They tell a
 * {@link http://en.wikipedia.org/wiki/Terminal_emulator terminal emulator} how to format the
 * characters, what color to display them in, etc. \phpseclib\File\ANSI is a
 * {@link http://en.wikipedia.org/wiki/VT100 VT100} terminal emulator.
 *
 * @category  File
 * @package   ANSI
		...
		for ($i = 0; $i < strlen($source); $i ++){
				...
				switch ($this->ansi){
					...
					case "\x1B[K": // Clear screen from cursor right
						...
						array_splice($this->attrs[$this->y], $this->x + 1, $this->max_x - $this->x,
							array_fill($this->x, $this->max_x - $this->x - 1,
								$this->base_attr_cell));
						break;
					...
					default:
						switch (TRUE){
							case preg_match('#\x1B\[(\d+)B#', $this->ansi,
								$match): // Move cursor down n lines
								$this->old_y = $this->y;
								$this->y     += $match[1];
								break;
							case preg_match('#\x1B\[(\d+);(\d+)H#', $this->ansi,
								...
								$this->y     = $match[1] - 1;
								break;
							case preg_match('#\x1B\[(\d+)C#', $this->ansi,
								$match): // Move cursor right n lines
								$this->old_x = $this->x;
								$this->x     += $match[1];
								break;
							case preg_match('#\x1B\[(\d+)D#', $this->ansi,
								$match): // Move cursor left n lines
								$this->old_x = $this->x;
								$this->x     -= $match[1];
								if ($this->x < 0){
									$this->x = 0;
							...
							case preg_match('#\x1B\[(\d+);(\d+)r#', $this->ansi,
								...
								break;
							case preg_match('#\x1B\[(\d*(?:;\d*)*)m#', $this->ansi,
								$match): // character attributes
			...
			switch ($source[$i]){
				...
				case "\x1B": // start ANSI escape code
					$this->tokenization[count($this->tokenization) - 1] = substr($this->tokenization[count($this->tokenization) - 1],
						0, - 1);
	...
	function _getScreen(){
		...
		for ($i = 0; $i <= $this->max_y; $i ++){
			for ($j = 0; $j <= $this->max_x; $j ++){
				...
				$output    .= $this->_processCoordinate($last_attr, $cur_attr,
	...
	function getHistory(){
		...
		for ($i = 0; $i < count($this->history); $i ++){
			for ($j = 0; $j <= $this->max_x + 1; $j ++){
				...
				$scrollback .= $this->_processCoordinate($last_attr, $cur_attr,
