...
class Stream{
			...
			if (isset(self::$instances[$host][$port][$user][(string) $pass])){
				...
				if (isset($this->notification) && is_callable($this->notification)){
					/* if !is_callable($this->notification) we could do this:
					   ...
					   user_error is on.
					*/
					call_user_func($this->notification, STREAM_NOTIFY_CONNECT,
						STREAM_NOTIFY_SEVERITY_INFO, '', 0, 0, 0);
					call_user_func($this->notification, STREAM_NOTIFY_AUTH_REQUIRED,
						STREAM_NOTIFY_SEVERITY_INFO, '', 0, 0, 0);
					if (!$this->sftp->login($user, $pass)){
						call_user_func($this->notification, STREAM_NOTIFY_AUTH_RESULT,
							STREAM_NOTIFY_SEVERITY_ERR, 'Login Failure',
							NET_SSH2_MSG_USERAUTH_FAILURE, 0, 0);

						return FALSE;
					}
					call_user_func($this->notification, STREAM_NOTIFY_AUTH_RESULT,
						STREAM_NOTIFY_SEVERITY_INFO, 'Login Success', NET_SSH2_MSG_USERAUTH_SUCCESS,
						0, 0);
				}else{
					if (!$this->sftp->login($user, $pass)){
						return FALSE;
	...
	function _stream_read($count){
		...
		if (isset($this->notification) && is_callable($this->notification)){
			if ($result === FALSE){
				call_user_func($this->notification, STREAM_NOTIFY_FAILURE,
					STREAM_NOTIFY_SEVERITY_ERR, $this->sftp->getLastSFTPError(), NET_SFTP_OPEN, 0,
					0);

				return 0;
			}
			// seems that PHP calls stream_read in 8k chunks
			call_user_func($this->notification, STREAM_NOTIFY_PROGRESS, STREAM_NOTIFY_SEVERITY_INFO,
				'', 0, strlen($result), $this->size);
		}
	 ...
	 *
	 * Only fgets / fread, however, results in feof() returning true. do fputs($fp, 'aaa') on a
	 * blank file and feof() will return false. do fread($fp, 1) and feof() will then return true.
	 * do fseek($fp, 10) on ablank file and feof() will return false. do fread($fp, 1) and feof()
	...
	function _rename($path_from, $path_to){
		...
		if (!$this->sftp->rename($path_from, $path_to)){
			if ($this->sftp->stat($path_to)){
				return $this->sftp->delete($path_to, TRUE) && $this->sftp->rename($path_from,
						$path_to);
			}
	 ...
	 *
	 * The only $options is "whether or not to enforce safe_mode (0x04)". Since safe mode was
	 * deprecated in 5.3 and removed in 5.4 I'm just going to ignore it.
	 *
	 * Also, nlist() is the best that this function is realistically going to be able to do. When
	 * an SFTP client sends a SSH_FXP_READDIR packet you don't generally get info on just one file
	 * but on multiple files. Quoting the SFTP specs:
	 *
	 ...
	 * Removes a directory
	 *
	 * Only valid $options is STREAM_MKDIR_RECURSIVE per <http://php.net/streamwrapper.rmdir>,
	 * however,
	 * <http://php.net/rmdir>  does not have a $recursive parameter as mkdir() does so I don't know
	 * how STREAM_MKDIR_RECURSIVE is supposed to be set. Also, when I try it out with rmdir() I get
	 * 8 as
	 ...
	 * Flushes the output
	 *
	 * See <http://php.net/fflush>. Always returns true because \phpseclib\Net\SFTP doesn't cache
	 * stuff before writing
	 *
	 * @return bool
	 * @access public
	 */
	 ...
	 *
	 * Ignores the STREAM_URL_STAT_QUIET flag because the entirety of \phpseclib\Net\SFTP\Stream is
	 * quiet by default might be worthwhile to reconstruct bits 12-16 (ie. the file type) if mode
	 * doesn't have them but we'll cross that bridge when and if it's reached
	 *
	 * @param string $path
	 * @param int $flags
	 *
	 ...
	 * __call Magic Method
	 *
	 * When you're utilizing an SFTP stream you're not calling the methods in this class directly -
	 * PHP is calling them for you. Which kinda begs the question... what methods is PHP calling
	 * and what parameters is it passing to them? This function lets you figure that out.
	 *
	 * If NET_SFTP_STREAM_LOGGING is defined all calls will be output on the screen and then
	 * (regardless of whether or not NET_SFTP_STREAM_LOGGING is enabled) the parameters will be
	 * passed through to the appropriate method.
	 *
	 * @param string
	 * @param array
	 *
