...
class SSH1{
	 ...
	 *
	 * Not supported nor is it defined in the official SSH1 specs.  OpenSSH, however, does define
	 * it (see cipher.h), although it doesn't use it (see cipher.c)
	 */
	 ...
	 *
	 *     This library currently only supports encryption when the same key is being used for both
	 *     directions.  This is because there's only one $crypto object.  Two could be added
	 ...
	 * Blowfish
	 *
	 * Not supported nor is it defined in the official SSH1 specs.  OpenSSH, however, defines it
	 * (see cipher.h) and uses it (see cipher.c)
	 */
	 ...
	 *
	 * Set by the constructor call. Calling setTimeout() is optional. If it's not called functions
	 * like exec() won't timeout unless some PHP setting forces it too. The timeout specified in
	 * the constructor, however, is non-optional. There will be a timeout, whether or not you set
	...
	function _connect(){
		...
		$server_key_public_exponent       = new BigInteger($this->_string_shift($response[self::RESPONSE_DATA],
			ceil($temp['len'] / 8)), 256);
		...
		$temp                      = unpack('nlen',
			$this->_string_shift($response[self::RESPONSE_DATA], 2));
		$server_key_public_modulus = new BigInteger($this->_string_shift($response[self::RESPONSE_DATA],
			ceil($temp['len'] / 8)), 256);
		...
		$temp                           = unpack('nlen',
			$this->_string_shift($response[self::RESPONSE_DATA], 2));
		$host_key_public_exponent       = new BigInteger($this->_string_shift($response[self::RESPONSE_DATA],
			ceil($temp['len'] / 8)), 256);
		...
		$temp                    = unpack('nlen',
			$this->_string_shift($response[self::RESPONSE_DATA], 2));
		$host_key_public_modulus = new BigInteger($this->_string_shift($response[self::RESPONSE_DATA],
			ceil($temp['len'] / 8)), 256);
	...
	function login($username, $password = ''){
		...
		if (defined('NET_SSH1_LOGGING') && NET_SSH1_LOGGING == self::LOG_COMPLEX){
			$data                                             = pack('CNa*',
				NET_SSH1_CMSG_AUTH_PASSWORD, strlen('password'), 'password');
	 ...
	 *
	 * An SSH1 server will close the connection after a command has been executed on a
	 * non-interactive shell.  SSH2 servers don't, however, this isn't an SSH2 client.  The way
	 * this works, on the server, is by initiating a shell with the -s option, as discussed in the
	 * following links:
	 *
	 ...
	 *
	 * Requires PHP 4.3.0 or later due to the use of the stream_select() function.  If you see
	 * stuff like
	 * "^[[00m", you're seeing ANSI escape codes.  According to
	 * {@link http://support.microsoft.com/kb/101875 How to Enable ANSI.SYS in a Command Window},
	 * "Windows NT does not support ANSI escape sequences in Win32 Console applications", so if
	 * you're a Windows user, there's not going to be much recourse.
	 *
	 * @return string
	 * @access public
	 * @see self::interactiveRead()
	 ...
	 * RSA Encrypt
	 *
	 * Returns mod(pow($m, $e), $n), where $n should be the product of two (large) primes $p and $q
	 * and where $e should be a number with the property that gcd($e, ($p - 1) * ($q - 1)) == 1.
	 * Could just make anything that calls this call modexp, instead, but I think this makes things
	 * clearer, maybe...
	 *
	 * @param BigInteger $m
	 * @param array $key
	 ...
	 *
	 * Returns a string if NET_SSH1_LOGGING == self::LOG_COMPLEX, an array if NET_SSH1_LOGGING ==
	 * self::LOG_SIMPLE and false if !defined('NET_SSH1_LOGGING')
	 *
	 * @access public
	 ...
	 *
	 * Returns, by default, the base-10 representation.  If $raw_output is set to true, returns,
	 * instead, the raw bytes.  This behavior is similar to PHP's md5() function.
	 *
	 ...
	 *
	 * Just because a cipher is supported by an SSH1 server doesn't mean it's supported by this
	 * library. If $raw_output is set to true, returns, instead, an array of constants.  ie.
	 * instead of array('Triple-DES in CBC mode'), you'll get array(self::CIPHER_3DES).
	 *
	 * @param bool $raw_output
	 *
	 * @return array
	 ...
	 * Just because a cipher is supported by an SSH1 server doesn't mean it's supported by this
	 * library. If $raw_output is set to true, returns, instead, an array of constants.  ie.
	 * instead of array('password authentication'), you'll get array(self::AUTH_PASSWORD).
	 *
	 * @param bool $raw_output
	 *
	 * @return array
