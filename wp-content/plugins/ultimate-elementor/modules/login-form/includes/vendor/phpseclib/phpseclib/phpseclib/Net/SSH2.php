...
class SSH2{
	 ...
	 * Channel constants
	 *
	 * RFC4254 refers not to client and server channels but rather to sender and recipient
	 * channels.  we don't refer to them in that way because RFC4254 toggles the meaning. the
	 * client sends a SSH_MSG_CHANNEL_OPEN message with a sender channel and the server sends a
	 * SSH_MSG_CHANNEL_OPEN_CONFIRMATION in response, with a sender and a recepient channel.  at
	 * first glance, you might conclude that SSH_MSG_CHANNEL_OPEN_CONFIRMATION's sender channel
	 * would be the same thing as SSH_MSG_CHANNEL_OPEN's sender channel, but it's not, per this
	 * snipet: The 'recipient channel' is the channel number given in the original open request,
	 * and 'sender channel' is the channel number allocated by the other side.
	 *
	 ...
	 * Execution Bitmap
	 *
	 * The bits that are set represent functions that have been called already.  This is used to
	 * determine if a requisite function has been successfully executed.  If not, an error should
	 * be thrown.
	 *
	 * @var int
	 * @access private
	 ...
	 *
	 * We need to know how big the HMAC will be for the server to client direction so that we know
	 * how many bytes to read. For the client to server side, the HMAC object will make the HMAC as
	 * long as it needs to be.  All we need to do is append it.
	 *
	 * @see self::_get_binary_packet()
	 * @var int
	 * @access private
	 ...
	 * Channel Buffers
	 *
	 * If a client requests a packet from one channel but receives two packets from another those
	 * packets should be placed in a buffer
	 *
	 * @see self::_get_channel_packet()
	 * @see self::exec()
	 * @var array
	 ...
	 * The Window Size
	 *
	 * Bytes the other party can send before it must wait for the window to be adjusted (0x7FFFFFFF
	 * = 2GB)
	 *
	 * @var int
	 * @see self::_send_channel_packet()
	 ...
	 *
	 * https://tools.ietf.org/html/rfc4253#section-4.2 says "when the connection has been
	 * established, both sides MUST send an identification string". It does not say which side
	 * sends it first. In theory it shouldn't matter but it is a fact of life that some SSH servers
	 * are simply buggy
	 *
	 * @access public
	 */
	...
	function _connect(){
		...
		if (!is_resource($this->fsock)){
			...
			$this->fsock = @fsockopen($this->host, $this->port, $errno, $errstr,
	...
	function _key_exchange($kexinit_payload_server = FALSE){
		...
		$this->_string_shift($response,
			1); // skip past the message number (it should be SSH_MSG_KEXINIT)
		...
		$decrypt          = $this->_array_intersect_first($s2c_encryption_algorithms,
		...
		$encrypt          = $this->_array_intersect_first($c2s_encryption_algorithms,
		...
		$temp                         = unpack('Nlength', $this->_string_shift($response, 4));
		$this->server_public_host_key = $server_public_host_key = $this->_string_shift($response,
			$temp['length']);
		...
		$mac_algorithm = $this->_array_intersect_first($c2s_mac_algorithms,
		...
		$mac_algorithm = $this->_array_intersect_first($s2c_mac_algorithms,
		...
		$compression_algorithm = $this->_array_intersect_first($c2s_compression_algorithms,
		...
		$compression_algorithm = $this->_array_intersect_first($s2c_compression_algorithms,
			$this->compression_algorithms_client_to_server);
		if ($compression_algorithm === FALSE){
	 ...
	 * Login
	 *
	 * The $password parameter can be a plaintext password, a \phpseclib\Crypt\RSA object or an
	 * array
	 *
	 ...
	 *     {@link http://tools.ietf.org/html/rfc4251#section-9.3.9 traffic analysis} by sending
	 *     dummy SSH_MSG_IGNORE messages.
	 */
	function _login_helper($username, $password = NULL){
		...
		switch ($type){
			case NET_SSH2_MSG_USERAUTH_PASSWD_CHANGEREQ: // in theory, the password can be changed
				...
				$this->errors[] = 'SSH_MSG_USERAUTH_PASSWD_CHANGEREQ: ' . $this->_string_shift($response,
						$length);
	 ...
	 *
	 * See {@link http://tools.ietf.org/html/rfc4256 RFC4256} for details.  This is not a
	 * full-featured keyboard-interactive authenticator.
	 *
	 * @param string $username
	 * @param string $password
	 *
		...
		switch ($type){
			case NET_SSH2_MSG_USERAUTH_INFO_REQUEST:
                ...
                */
				...
				$packet = $logged = pack('CN', NET_SSH2_MSG_USERAUTH_INFO_RESPONSE,
					count($responses));
				for ($i = 0; $i < count($responses); $i ++){
	...
	function _privatekey_login($username, $privatekey){
		...
		$part2 = pack('Na*Na*', strlen($signatureType), $signatureType, strlen($publickey),
			$publickey);
		...
		switch ($type){
			case NET_SSH2_MSG_USERAUTH_FAILURE:
				...
				extract(unpack('Nlength', $this->_string_shift($response, 4)));
				$this->errors[] = 'SSH_MSG_USERAUTH_FAILURE: ' . $this->_string_shift($response,
						$length);
		...
		$signature = $privatekey->sign(pack('Na*a*', strlen($this->session_id), $this->session_id,
			$packet));
		$signature = pack('Na*Na*', strlen($signatureType), $signatureType, strlen($signature),
			$signature);
	 ...
	 * Set Timeout
	 *
	 * $ssh->exec('ping 127.0.0.1'); on a Linux host will never return and will run indefinitely.
	 * setTimeout() makes it so it'll timeout. Setting $timeout to false or 0 will mean there is no
	 * timeout.
	 *
	 * @param mixed $timeout
	 *
	 ...
	 * \phpseclib\Net\SSH2::_get_channel_packet(self::CHANNEL_EXEC) will need to be called
	 * manually. In all likelihood, this is not a feature you want to be taking advantage of.
	 *
	 ...
	 * Start a subsystem.
	 *
	 * Right now only one subsystem at a time is supported. To support multiple subsystem's
	 * stopSubsystem() could accept a string that contained the name of the subsystem, but at that
	 * point, only one subsystem of each type could be opened. To support multiple subsystem's of
	 * the same name maybe it'd be best if startSubsystem() generated a new channel id and returns
	 * that and then that that was passed into stopSubsystem() but that'll be saved for a future
	 ...
	 * Closes a channel
	 *
	 * If read() timed out you might want to just close the channel and have it auto-restart on the
	 * next read() call
	 *
	 * @access public
	 */
	 ...
	 * you'll need to call disconnect().
	 *
	 * @access public
	 */
	function __destruct(){
	...
	function _get_binary_packet($skip_channel_filter = FALSE){
		...
		if ($this->hmac_check !== FALSE){
			...
			if ($hmac === FALSE || strlen($hmac) != $this->hmac_size){
				...
				return FALSE;
			}elseif ($hmac != $this->hmac_check->hash(pack('NNCa*', $this->get_seq_no,
		...
		if (defined('NET_SSH2_LOGGING')){
			...
			$message_number = '<- ' . $message_number .
					...
					4) . ', network: ' . round($stop - $start, 4) . 's)';
		...
		if (($this->bitmap & self::MASK_CONNECTED) && $this->isAuthenticated()){
			switch (ord($payload[0])){
				...
				case NET_SSH2_MSG_GLOBAL_REQUEST: // see http://tools.ietf.org/html/rfc4254#section-4
					...
					extract(unpack('Nlength', $this->_string_shift($payload, 4)));
					$this->errors[] = 'SSH_MSG_GLOBAL_REQUEST: ' . $this->_string_shift($payload,
							$length);
			...
			if (isset($channel) && isset($this->channel_status[$channel]) && isset($this->window_size_server_to_client[$channel])){
				...
				if ($this->window_size_server_to_client[$channel] < 0){
					$packet = pack('CNN', NET_SSH2_MSG_CHANNEL_WINDOW_ADJUST,
						$this->server_channels[$channel], $this->window_size);
					if (!$this->_send_binary_packet($packet)){
						return FALSE;
					}
	...
	function _send_binary_packet($data, $logged = NULL){
		...
		$hmac = $this->hmac_create !== FALSE ? $this->hmac_create->hash(pack('Na*',
			$this->send_seq_no, $packet)) : '';
		$this->send_seq_no ++;
	 ...
	 *
	 * \phpseclib\Net\SSH2 doesn't properly close most channels.  For exec() channels are normally
	 * closed by the server and for SFTP channels are presumably closed when the client
	 * disconnects.  This functions is intended for SCP more than anything.
	 *
	 * @param int $client_channel
	 ...
	 * Define Array
	 *
	 * Takes any number of arrays whose indices are integers and whose values are strings and
	 * defines a bunch of named constants from it, using the value as the name of the constant and
	 * the index as the value of the constant. If any of the constants that would be defined
	 * already exists, none of the constants will be defined.
	 *
	 * @param array $array
	 *
	 * @access private
	 ...
	 *
	 * Returns a string if NET_SSH2_LOGGING == self::LOG_COMPLEX, an array if NET_SSH2_LOGGING ==
	 * self::LOG_SIMPLE and false if !defined('NET_SSH2_LOGGING')
	 *
	 * @access public
	...
	function _format_log($message_log, $message_number_log){
		...
		for ($i = 0; $i < count($message_log); $i ++){
			...
			do{
				...
				$hex      = substr(preg_replace_callback('#.#s', [$this, '_format_log_helper'],
					$fragment), strlen($this->log_boundary));
				// replace non ASCII printable characters with dots
				// http://en.wikipedia.org/wiki/ASCII#ASCII_printable_characters
				// also replace < with a . since < messes up the output on web browsers
				$raw    = preg_replace('#[^\x20-\x7E]|<#', '.', $fragment);
				$output .= str_pad($hex, $this->log_long_width - $this->log_short_width,
						' ') . $raw . "\r\n";
				$j ++;
	...
	/**
	 * Return a list of the (symmetric key) encryption algorithms the server supports, when
	...
	/**
	 * Return a list of the (symmetric key) encryption algorithms the server supports, when sending
	 * stuff to the client.
	 *
	 * @return array
	 * @access public
	...
	/**
	 * Return a list of the MAC algorithms the server supports, when receiving stuff from the
	 * client.
	 *
	 * @return array
	 * @access public
	...
	/**
	 * Return a list of the compression algorithms the server supports, when receiving stuff from
	 * the client.
	 *
	 * @return array
	 * @access public
	...
	/**
	 * Return a list of the compression algorithms the server supports, when sending stuff to the
	 * client.
	 *
	 * @return array
	 * @access public
	 ...
	 *
	 * Caching this the first time you connect to a server and checking the result on subsequent
	 * connections is recommended.  Returns false if the server signature is not signed correctly
	...
	function getServerPublicHostKey(){
		...
		switch ($this->signature_format){
			case 'ssh-dss':
				...
				$temp = unpack('Nlength', $this->_string_shift($server_public_host_key, 4));
				$p    = new BigInteger($this->_string_shift($server_public_host_key,
					$temp['length']), - 256);
				...
				$temp = unpack('Nlength', $this->_string_shift($server_public_host_key, 4));
				$q    = new BigInteger($this->_string_shift($server_public_host_key,
					$temp['length']), - 256);
				...
				$temp = unpack('Nlength', $this->_string_shift($server_public_host_key, 4));
				$g    = new BigInteger($this->_string_shift($server_public_host_key,
					$temp['length']), - 256);
				...
				$temp = unpack('Nlength', $this->_string_shift($server_public_host_key, 4));
				$y    = new BigInteger($this->_string_shift($server_public_host_key,
					$temp['length']), - 256);
			...
			case 'rsa-sha2-512':
				...
				$temp = unpack('Nlength', $this->_string_shift($server_public_host_key, 4));
				$e    = new BigInteger($this->_string_shift($server_public_host_key,
					$temp['length']), - 256);
                ...
                */
				...
				switch ($this->signature_format){
					case 'rsa-sha2-512':
						$h = pack('N5a*', 0x00305130, 0x0D060960, 0x86480165, 0x03040203,
							0x05000440, $hashObj->hash($this->exchange_hash));
						break;
					case 'rsa-sha2-256':
						$h = pack('N5a*', 0x00303130, 0x0D060960, 0x86480165, 0x03040201,
							0x05000420, $hashObj->hash($this->exchange_hash));
						break;
					//case 'ssh-rsa':
					default:
						$hash = 'sha1';
						$h    = pack('N4a*', 0x00302130, 0x0906052B, 0x0E03021A, 0x05000414,
