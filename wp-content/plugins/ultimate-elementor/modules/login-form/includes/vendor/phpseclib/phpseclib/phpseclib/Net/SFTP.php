 ...
 * PHP version 5
 *
 * Currently only supports SFTPv2 and v3, which, according to wikipedia.org, "is the most widely
 * used version, implemented by the popular OpenSSH SFTP server".  If you want SFTPv4/5/6 support,
 * provide me with access to an SFTPv4/5/6 server.
 *
...
class SFTP extends SSH2{
	 ...
	 * The Request ID
	 *
	 * The request ID exists in the off chance that a packet is sent out-of-order.  Of course, this
	 * library doesn't support concurrent actions, so it's somewhat academic, here.
	 *
	 * @var boolean
	 ...
	 * Stat Cache
	 *
	 * Rather than always having to open a directory and close it immediately there after to see if
	 * a file is a directory we'll cache the results.
	 *
	 ...
	 *
	 * SFTP doesn't provide a mechanism by which the current working directory can be changed, so
	 * we'll emulate it.  Returns the absolute (canonicalized) path.
	 *
	 * If canonicalize_paths has been disabled using disablePathCanonicalization(), $path is
	 * returned as-is.
	 *
	 * @param string $path
	 *
			...
			switch ($this->packet_type){
				case NET_SFTP_NAME:
					...
					$this->_string_shift($response,
						4); // skip over the count - it should be 1, anyway
					if (strlen($response) < 4){
						return FALSE;
					}
					extract(unpack('Nlength', $this->_string_shift($response, 4)));
	...
	function _nlist_helper($dir, $recursive, $relativeDir){
		...
		foreach ($files as $value){
			...
			if (is_array($this->_query_stat_cache($this->_realpath($dir . '/' . $value)))){
				$temp   = $this->_nlist_helper($dir . '/' . $value, TRUE,
					$relativeDir . $value . '/');
	 ...
	 *
	 * By default, \phpseclib\Net\SFTP::put() does not read from the local filesystem.  $data is
	 * dumped directly into $remote_file. So, for example, if you set $data to 'filename.ext' and
	 * then do \phpseclib\Net\SFTP::get(), you will get a file, twelve bytes long, containing
	 * 'filename.ext' as its contents.
	 *
	 * Setting $mode to self::SOURCE_LOCAL_FILE will change the above behavior.  With
	 * self::SOURCE_LOCAL_FILE, $remote_file will contain as many bytes as filename.ext does on
	 * your local filesystem.  If your filename.ext is 1MB then that is how large $remote_file will
	 * be, as well.
	 *
	 * Setting $mode to self::SOURCE_CALLBACK will use $data as callback function, which gets only
	 * one parameter -- number of bytes to return, and returns a string if there is some data or
	 * null if there is no more data
	 *
	 * If $data is a resource then it'll be used as a resource instead.
	 *
	 * Currently, only binary mode is supported.  As such, if the line endings need to be adjusted,
	 * you will need to take care of that, yourself.
	 *
	 * $mode can take an additional two parameters - self::RESUME and self::RESUME_START. These are
	 * bitwise AND'd with
	 * $mode. So if you want to resume upload of a 300mb file on the local file system you'd set
	 * $mode to the following:
	 *
	 * self::SOURCE_LOCAL_FILE | self::RESUME
	 *
	 * If you wanted to simply append the full contents of a local file to the full contents of a
	 * remote file you'd replace self::RESUME with self::RESUME_START.
	 *
	 * If $mode & (self::RESUME | self::RESUME_START) then self::RESUME_START will be assumed.
	 *
	 * $start and $local_start give you more fine grained control over this process and take
	 * precident over self::RESUME when they're non-negative. ie. $start could let you write at the
	 * end of a file (like self::RESUME) or in the middle of one. $local_start could let you start
	 * your reading from the end of a file (like self::RESUME_START) or in the middle of one.
	 *
	 ...
	 * @return bool
	 * @access public
	 * @internal ASCII mode for SFTPv4/5/6 can be supported by adding a new function -
	 *     \phpseclib\Net\SFTP::setMode().
	 */
	function put(
		$remote_file,
		$data,
		...
		while ($dataCallback || ($size === 0 || $sent < $size)){
			if ($dataCallback){
				...
				$temp = isset($fp) ? fread($fp, $sftp_packet_size) : substr($data, $sent,
					$sftp_packet_size);
				if ($temp === FALSE || $temp === ''){
					break;
				}
			...
			$subtemp = $offset + $sent;
			$packet  = pack('Na*N3a*', strlen($handle), $handle, $subtemp / 4294967296, $subtemp,
				strlen($temp), $temp);
	 ...
	 *
	 * Sending an SSH_FXP_WRITE packet and immediately reading its response isn't as efficient as
	 * blindly sending out $i SSH_FXP_WRITEs, in succession, and then reading $i responses.
	 *
	 * @param int $i
	 *
	 * @return bool
	 ...
	 *
	 * Returns a string containing the contents of $remote_file if $local_file is left undefined or
	 * a boolean false if the operation was unsuccessful.  If $local_file is defined, returns true
	...
	function get(
		...
		while (TRUE){
			...
			while ($i < NET_SFTP_QUEUE_SIZE && ($length < 0 || $read < $length)){
				...
				$packet_size = $length > 0 ? min($this->max_sftp_packet,
					$length - $read) : $this->max_sftp_packet;

				$packet = pack('Na*N3', strlen($handle), $handle, $tempoffset / 4294967296,
					$tempoffset, $packet_size);
	 ...
	 *
	 * Quoting the SFTP RFC, "Implementations MUST NOT send bits that are not defined" but they
	 * seem to anyway
	 *
	 * @param int $mode
	 *
	 ...
	 * See '6. General Packet Format' of draft-ietf-secsh-filexfer-13 for more info.
	 *
	 * Incidentally, the number of SSH_MSG_CHANNEL_DATA messages has no bearing on the number of
	 * SFTP packets present. There can be one SSH_MSG_CHANNEL_DATA messages containing two SFTP
	 * packets or there can be two SSH_MSG_CHANNEL_DATA messages containing one SFTP packet.
	 *
	 * @return string
	 * @access private
	 * @see self::_send_sftp_packet()
