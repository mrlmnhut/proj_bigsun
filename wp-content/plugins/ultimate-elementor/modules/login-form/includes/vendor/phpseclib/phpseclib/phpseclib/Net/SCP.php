...
class SCP{
	 ...
	 *
	 * By default, \phpseclib\Net\SCP::put() does not read from the local filesystem.  $data is
	 * dumped directly into $remote_file. So, for example, if you set $data to 'filename.ext' and
	 * then do \phpseclib\Net\SCP::get(), you will get a file, twelve bytes long, containing
	...
	function get($remote_file, $local_file = FALSE){
		...
		if (!preg_match('#(?<perms>[^ ]+) (?<size>\d+) (?<name>.+)#', rtrim($this->_receive()),
			$info)){
			return FALSE;
		}
				...
				while (TRUE){
					...
					switch ($response[SSH1::RESPONSE_TYPE]){
						case NET_SSH1_SMSG_STDOUT_DATA:
							...
							return $this->ssh->_string_shift($response[SSH1::RESPONSE_DATA],
								$length);
