 ...
 *
 * Display PHP error/debug log messages in Google Chrome console and notification popups, executes
 * PHP code remotely
 *
 * Usage:
...
class PHPConsoleHandler extends AbstractProcessingHandler{
	...
	private function handleDebugRecord(array $record){
		...
		if ($record['context']){
			$message .= ' ' . Utils::jsonEncode($this->connector->getDumper()
			                                                    ->dump(array_filter($record['context'])),
					NULL, TRUE);
		}
		$this->connector->getDebugDispatcher()
		                ->dispatchDebug($message, $tags,
