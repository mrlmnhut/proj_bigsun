...
class Module extends Module_Base{
	...
	public function __construct(){
		...
		add_action('wp_ajax_uael_register_user', [$this, 'get_form_data']);
		add_action('wp_ajax_nopriv_uael_register_user', [$this, 'get_form_data']);
		add_filter('wp_new_user_notification_email',
			[$this, 'custom_wp_new_user_notification_email'], 10, 3);
	}
	 ...
	 * @since 1.18.0
	 */
	public function custom_wp_new_user_notification_email(
		$wp_new_user_notification_email,
		$user,
		$blogname){

		if (array_key_exists('template_type',
				self::$email_content) && 'custom' === self::$email_content['template_type']){

			$wp_new_user_notification_email['subject'] = sprintf(self::$email_content['subject'],
				$blogname, $user->user_login);

			$message = self::$email_content['message'];

			$find = ['/\[field=password\]/', '/\[field=username\]/', '/\[field=email\]/', '/\[field=first_name\]/', '/\[field=last_name\]/'];

			$replacement = [self::$email_content['pass'], self::$email_content['user_login'], self::$email_content['user_email'], self::$email_content['first_name'], self::$email_content['last_name']];
	...
	public function get_form_data(){
		...
		if (isset($_POST['data'])){
			...
			if (isset($data['is_recaptcha_enabled'])){
				if ('yes' === sanitize_text_field($data['is_recaptcha_enabled'])){
					...
					if (empty($recaptcha_token)){
						$error['recaptcha'] = __('The Captcha field cannot be blank. Please enter a value.',
							'uael');
					}
					...
					if (200 !== (int) $response_code){
						...
						$error['recaptcha'] = sprintf(__('Can not connect to the reCAPTCHA server (%d).',
							'uael'), $response_code);
					}else{
			...
			if (isset($data['email']) && !is_email($data['email'])){
				...
				$error['email'] = __('An account is already registered with your email address. Please choose another one.',
					'uael');
			}
			...
			if (isset($data['user_name']) && !empty($data['user_name']) && !validate_username($data['user_name'])){
				...
				$error['user_name'] = __('This username is invalid because it uses illegal characters. Please enter a valid username.',
					'uael');
				...
				$error['user_name'] = __('This username is already registered. Please choose another one.',
					'uael');

			}elseif (isset($data['user_name']) && !empty($data['user_name'])){

				/** This Filters the list of blacklisted usernames. */
				$illegal_logins = (array) apply_filters('uael_illegal_user_logins', []);

				if (in_array(strtolower($data['user_name']),
					array_map('strtolower', $illegal_logins), TRUE)){
			...
			$user_login = (isset($data['user_name']) && !empty($data['user_name'])) ? sanitize_user($data['user_name'],
				TRUE) : '';
