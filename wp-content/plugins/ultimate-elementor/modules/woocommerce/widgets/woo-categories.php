...
class Woo_Categories extends Common_Widget{
		...
		if (parent::is_internal_links()){
			...
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf(__('%1$s Getting started article » %2$s', 'uael'),
						'<a href="https://uaelementor.com/docs/woo-categories-widget/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">',
						'</a>'),
			...
			$this->add_control(
				...
				[
					'type'            => Controls_Manager::RAW_HTML,
					/* translators: %1$s doc link */
					'raw'             => sprintf(__('%1$s How to set description for category? » %2$s',
						'uael'),
						'<a href="https://uaelementor.com/docs/how-to-set-description-for-category-in-woocommerce/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">',
						'</a>'),
					'content_classes' => 'uael-editor-doc',
				]
			);
	...
	public function query_product_categories(){
		...
		add_action('woocommerce_before_subcategory', [$this, 'template_loop_category_link_open'],
			10);

		/* Category Wrapper */
		add_action('woocommerce_before_subcategory', [$this, 'category_wrap_start'], 15);
		add_action('woocommerce_after_subcategory', [$this, 'category_wrap_end'], 8);

		if ('yes' === $settings['display_cat_desc']){
			add_action('woocommerce_after_subcategory', [$this, 'category_description'], 8);
		}

		/* Category Title */
		remove_action('woocommerce_shop_loop_subcategory_title',
			'woocommerce_template_loop_category_title', 10);
		add_action('woocommerce_shop_loop_subcategory_title',
			[$this, 'template_loop_category_title'], 10);
		...
		/* Category Link */
		add_action('woocommerce_before_subcategory', 'woocommerce_template_loop_category_link_open',
			10);
		remove_action('woocommerce_before_subcategory', [$this, 'template_loop_category_link_open'],
			10);

		/* Category Wrapper */
		remove_action('woocommerce_before_subcategory', [$this, 'category_wrap_start'], 15);
		remove_action('woocommerce_after_subcategory', [$this, 'category_wrap_end'], 8);
	...
	function template_loop_category_link_open($category){
		$link = apply_filters('uael_woo_category_link',
	...
	public function template_loop_category_title($category){
		...
		if ($category->count > 0){
			...
			/* translators: 1: number of products */
				_nx('<mark class="uael-count">%1$s Product</mark>',
					'<mark class="uael-count">%1$s Products</mark>', $category->count,
					'product categories', 'uael'),
