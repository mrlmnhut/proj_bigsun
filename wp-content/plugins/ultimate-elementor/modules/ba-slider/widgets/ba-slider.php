...
class BaSlider extends Common_Widget{
			...
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf(__('%1$s Getting started video » %2$s', 'uael'),
						'<a href="https://www.youtube.com/watch?v=7m6FD8Yk3N0&list=PL1kzJGWGPrW_7HabOZHb6z88t_S8r-xAc&index=3" target="_blank" rel="noopener">',
						'</a>'),
			...
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf(__('%1$s Getting started article » %2$s', 'uael'),
						'<a href="https://uaelementor.com/docs/before-after-slider-widget/?utm_source=uael-pro-dashboard&utm_medium=uael-editor-screen&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">',
						'</a>'),
					'content_classes' => 'uael-editor-doc',
				]
			);
	...
	protected function get_image_src($position){
		...
		if ('media' === $settings[$position . '_src']){
			...
			if ('' !== $settings[$position . '_image']['id']){
				...
				$url = Group_Control_Image_Size::get_attachment_image_src($settings[$position . '_image']['id'],
					$position . '_image', $settings);
			}else{
	...
	protected function _content_template(){
		?>
        <#        var before_img = '';        var after_img = '';

        if( 'media' == settings.before_src ) {

        var before_image = {                id: settings.before_image.id,                url: settings.before_image.url,                size: settings.before_image_size,                dimension: settings.before_image_custom_dimension,                model: view.getEditModel()            };            before_img = elementor.imagesManager.getImageUrl( before_image );        } else {            before_img = settings.before_img_url;        }

        if( 'media' == settings.after_src ) {            var after_image = {                id: settings.after_image.id,                url: settings.after_image.url,                size: settings.after_image_size,                dimension: settings.after_image_custom_dimension,                model: view.getEditModel()            };            after_img = elementor.imagesManager.getImageUrl( after_image );        } else {            after_img = settings.after_img_url;        }

        if ( ! before_img || ! after_img ) {            return;        }
        ...
        </div>        <# elementorFrontend.hooks.doAction( 'frontend/element_ready/uael-ba-slider.default' ); #>
		<?php
	}
