...
class Fancy_Heading extends Common_Widget{
			...
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf(__('%1$s Getting started video » %2$s', 'uael'),
						'<a href="https://www.youtube.com/watch?v=BqXOvmpulQQ&index=7&list=PL1kzJGWGPrW_7HabOZHb6z88t_S8r-xAc" target="_blank" rel="noopener">',
						'</a>'),
			...
			$this->add_control(
				...
				[
					...
					'raw'             => sprintf(__('%1$s Getting started article » %2$s', 'uael'),
						'<a href="https://uaelementor.com/docs/fancy-heading-widget/?utm_source=uael-pro-dashboard&utm_medium=uael-menu-page&utm_campaign=uael-pro-plugin" target="_blank" rel="noopener">',
						'</a>'),
					'content_classes' => 'uael-editor-doc',
				]
			);
	...
	protected function render(){
        ...
        <div class="uael-module-content uael-fancy-text-node" <?php echo $this->get_render_attribute_string('fancy-text'); ?>>
			<?php if (!empty($settings['fancytext_effect_type'])){ ?><?php echo '<' . $settings['fancytext_title_tag']; ?> class="uael-fancy-text-wrap uael-fancy-text-<?php echo $settings['fancytext_effect_type']; ?>">
				...
				<?php } ?><?php echo '</' . $settings['fancytext_title_tag'] . '>'; ?><?php } ?>
        </div>
		<?php
	}
	...
	protected function _content_template(){
		?>
        <#        function get_fancytext_data(){            var ipstr        = settings.fancytext;            var strs        = ipstr.split( "\n" );            return strs;        }        var effect_type = settings.fancytext_effect_type;

        if ( 'type' == settings.fancytext_effect_type ) {            var type_speed  = ( '' != settings.fancytext_type_speed.size ) ? settings.fancytext_type_speed.size : 120;            var back_speed  = ( '' != settings.fancytext_type_backspeed.size ) ? settings.fancytext_type_backspeed.size : 60;            var start_delay = ( '' != settings.fancytext_type_start_delay.size ) ? settings.fancytext_type_start_delay.size : 0;            var back_delay  = ( '' != settings.fancytext_type_back_delay.size ) ? settings.fancytext_type_back_delay.size : 1200;            var loop        = ( 'yes' == settings.fancytext_type_loop ) ? 'true' : 'false';

        if ( 'yes' == settings.fancytext_type_show_cursor ) {                var show_cursor = 'true';                var cursor_char = ( '' != settings.fancytext_type_cursor_text ) ? settings.fancytext_type_cursor_text : '|';            } else {                var show_cursor = 'false';                var cursor_char = '';            }

        var data_strings = get_fancytext_data();            var fancy_data = JSON.stringify( data_strings );

        view.addRenderAttribute(                'fancy-text',                {                    'data-type-speed'        : type_speed,                    'data-animation'        : effect_type,                    'data-back-speed'        : back_speed,                    'data-start-delay'        : start_delay,                    'data-back-delay'        : back_delay,                    'data-loop'            : loop,                    'data-show-cursor'    : show_cursor,                    'data-cursor-char'        : cursor_char,                    'data-strings'            : fancy_data,                }            );        }        else if ( 'slide' == settings.fancytext_effect_type ) {
        ...
        view.addRenderAttribute(                'fancy-text',                {                    'data-animation'        : effect_type,                    'data-speed'            : speed,                    'data-pause'            : pause,                    'data-mousepause'        : mousepause,                    'data-strings'            : fancy_data,                }            );        }        #>
        <div class="uael-module-content uael-fancy-text-node" {{{ view.getRenderAttributeString( 'fancy-text' ) }}}>                <# if ( '' != settings.fancytext_effect_type ) { #>                    <{{{ settings.fancytext_title_tag }}} class="uael-fancy-text-wrap uael-fancy-text-{{{ settings.fancytext_effect_type }}}" >
        ...
        <span class="uael-fancy-stack">
							...
							<# } #>
						</span>                        <# if ( '' != settings.fancytext_suffix ) { #>
        <span class="uael-fancy-heading uael-fancy-text-suffix">{{{ settings.fancytext_suffix }}}</span>                        <# } #>

        </{{{ settings.fancytext_title_tag }}}>                <# } #>            </div>            <# elementorFrontend.hooks.doAction( 'frontend/element_ready/uael-fancy-heading.default' ); #>
		<?php
	}
