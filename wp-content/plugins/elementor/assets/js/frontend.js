...
var EventManager = function EventManager() {
  var slice = Array.prototype.slice,
      MethodsAvailable;
  /**
   * Contains the hooks that get registered with this EventManager. The array
   * for storage utilizes a "flat" object literal such that looking up the hook
   * utilizes the native object literal hash.
   */
  ...
  /**
   * Use an insert sort for keeping our hooks organized based on priority. This
   * function is ridiculously faster than bubble sort, etc:
   * http://jsperf.com/javascript-sort
   *
   * @param hooks The custom array containing all of the appropriate hooks to
   *     perform an insert sort on.
   * @private
   */
  ...
  /**
   * Runs the specified hook. If it is an action, the value is not modified but
   * if it is a filter, it is.
   *
   * @param type 'actions' or 'filters'
   * @param hook The hook ( namespace.identifier ) to be ran.
   * @param args Arguments to pass to the action/filter. If it's a filter, args
   *     is actually a single parameter.
   * @private
   */
   ...
   * @param callback Must be a valid callback function before this action is
   *     added
   * @param [priority=10] Used to control when the function is executed in
   *     relation to other callbacks bound to the same hook
  ...
  /**
   * Performs an action if it exists. You can pass as many arguments as you
   * want to this function; the only rule is that the first argument must
   * always be the action.
   */
  ...
  /**
   * Performs a filter if it exists. You should only ever pass 1 argument to be
   * filtered. The only rule is that the first argument must always be the
   * filter.
   */
  ...
  /**
   * Maintain a reference to the object scope so our public methods never get
   * confusing.
   */
