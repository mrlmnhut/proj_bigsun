...
abstract class Widget_Base extends Element_Base {
	...
	public function add_lightbox_data_to_image_link($link_html, $id){
		...
		return preg_replace('/^<a/', '<a ' . $this->get_render_attribute_string('link'),
			$link_html);
	}
	 ...
	 *
	 */
	public function add_lightbox_data_attributes(
		$element,
		$id = NULL,
		$lightbox_setting_key = NULL,
		$group_id = NULL,
		$overwrite = FALSE){
		$general_settings_model           = SettingsManager::get_settings_managers('general')
		                                                   ->get_model();
