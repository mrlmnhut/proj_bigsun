...
class WPSEO_Frontend {
	...
	public function get_robots() {
		...
		if ( is_object( $post ) && WPSEO_Frontend_Page_Type::is_simple_page() ) {
			...
			elseif ( is_author() ) {
				...
				$user_has_posts = ((int) count_user_posts($curauth->ID,
						$author_archive->get_author_archive_post_types(), TRUE)) > 0;
	...
	public function flush_cache() {
		...
		if ($head_end && $this->show_debug_marker()){
			...
			$head = str_replace($debug_mark,
				$debug_mark . PHP_EOL . '<title>' . esc_html($title) . '</title>', $head);
