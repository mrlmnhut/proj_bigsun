...
class WPSEO_Health_Check_Default_Tagline extends WPSEO_Health_Check{
	...
	public function run(){
		if (!$this->has_default_tagline()){
			$this->label          = esc_html__('You changed the default WordPress tagline',
				'wordpress-seo');
			$this->status         = self::STATUS_GOOD;
			$this->badge['color'] = 'blue';
			$this->description    = esc_html__('You are using a custom tagline or an empty one.',
				'wordpress-seo');

			return;
		}

		$this->label          = esc_html__('You should change the default WordPress tagline',
			'wordpress-seo');
		$this->status         = self::STATUS_RECOMMENDED;
		$this->badge['color'] = 'red';

		$this->description = esc_html__('You still have the default WordPress tagline. Even an empty one is probably better.',
			'wordpress-seo');
