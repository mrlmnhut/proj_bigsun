...
class WPSEO_Health_Check_Page_Comments extends WPSEO_Health_Check{
	...
	public function run(){
		if (!$this->has_page_comments()){
			$this->label          = esc_html__('Comments are displayed on a single page',
				'wordpress-seo');
			$this->status         = self::STATUS_GOOD;
			$this->badge['color'] = 'blue';
			$this->description    = esc_html__('Comments on your posts are displayed on a single page. This is just like we\'d suggest it. You\'re doing well!',
				'wordpress-seo');
		...
		$this->description    = esc_html__('Comments on your posts break into multiple pages. As this is not needed in 999 out of 1000 cases, we recommend you disable it. To fix this, uncheck "Break comments into pages..." on the Discussion Settings page.',
			'wordpress-seo');
