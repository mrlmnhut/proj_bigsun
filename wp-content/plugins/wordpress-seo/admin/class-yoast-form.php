...
class Yoast_Form {
	...
	public function checkbox( $var, $label, $label_left = false ) {
		...
		echo '<input class="checkbox ', esc_attr($class), '" type="checkbox" id="', esc_attr($var), '" name="', esc_attr($this->option_name), '[', esc_attr($var), ']" value="on"', checked($val,
			'on', FALSE), disabled($this->is_control_disabled($var), TRUE, FALSE), '/>';
	...
	public function light_switch(
		...
		'<input type="checkbox" aria-labelledby="', esc_attr($var . '-label'), '" id="', esc_attr($var), '" name="', esc_attr($this->option_name), '[', esc_attr($var), ']" value="on"', checked($val,
			'on', FALSE), disabled($this->is_control_disabled($var), TRUE, FALSE), '/>',
	...
	public function radio( $var, $values, $legend = '', $legend_attr = [] ) {
		...
		foreach ( $values as $key => $value ) {
			...
			echo '<input type="radio" class="radio" id="' . $var_esc . '-' . $key_esc . '" name="' . esc_attr($this->option_name) . '[' . $var_esc . ']" value="' . $key_esc . '" ' . checked($val,
					$key_esc, FALSE) . disabled($this->is_control_disabled($var), TRUE,
					FALSE) . ' />';
			$this->label(
				$label,
				[
	...
	public function toggle_switch( $var, $values, $label, $help = '' ) {
		...
		foreach ( $values as $key => $value ) {
			...
			$key_esc = esc_attr( $key );
			$for     = $var_esc . '-' . $key_esc;
			echo '<input type="radio" id="' . $for . '" name="' . esc_attr($this->option_name) . '[' . $var_esc . ']" value="' . $key_esc . '" ' . checked($val,
					$key_esc, FALSE) . disabled($this->is_control_disabled($var), TRUE,
					FALSE) . ' />',
